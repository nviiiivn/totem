import type { AgentConfig as SDKAgentConfig } from '@totem-ai/sdk/v2';
 ...
 * and clears config.model so Totem does not pre-resolve a stale value.
 */
...
export function createAgents(
  ...
  const protoCustomAgents = customAgentNames.flatMap((name) => {
    ...
    if (!hasCustomAgentModel(override)) {
      console.warn(
        `[totempole] Custom agent '${name}' skipped: 'model' is required`,
      );
      return [];
    }
  ...
  // See https://github.com/alvinunreal/totempole/issues/369
  ...
  const acpOrchestratorPrompts = acpSubAgents.map((agent) => {
    ...
    return [
      ...
      '- **Delegate when:** The user explicitly asks for this ACP-backed agent, or the task matches its role and benefits from software/subscription-specific capabilities outside Totem.',
...
/**
 * Get agent configurations formatted for the Totem SDK.
