...
- **User overrides**: From `~/.config/totem/totempole.json` via `loadAgentPrompt()`
...
4. **Configuration export**: `getAgentConfigs()` converts `AgentDefinition` to Totem SDK format with classification metadata
...
1. **Agent selection**: Totem selects the orchestrator as the primary agent
