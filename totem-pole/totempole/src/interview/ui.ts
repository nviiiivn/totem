...
const BRAND_LOGO_URL =
  'https://ohmytotemslim.com/android-chrome-512x512.png';
...
function brandImage(size: number): string {
  return `<img class="brand-mark" src="${BRAND_LOGO_URL}" alt="Oh My Totem Slim" width="${size}" height="${size}" />`;
}
...
export function renderDashboardPage(
  ...
  const emptyState =
    totalCount === 0
      ? '<p class="muted" style="text-align:center;padding:48px 0">No interviews yet. Start one with <code>/interview</code> in your Totem session.</p>'
      : '';
  ...
  const fileSection =
    unrecoveredFiles.length > 0
      ? `<h2>Files without session</h2><p class="muted file-hint">Resume with the command in any Totem session.</p>${filesHtml}`
      : '';
...
<html lang="en">
  ...
  <body>
    <div class="wrap">
      ...
      <div class="info-box">
        <strong>Interviews</strong> - live sessions and recovered files. State is pushed from Totem sessions to this dashboard.<br>
      ...
      ${activeSection}
      ${fileSection}
      <div class="footer">OH MY TOTEM SLIM</div>
    </div>
