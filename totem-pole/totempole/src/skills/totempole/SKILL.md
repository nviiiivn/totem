---
name: totempole
description: Configure and improve totempole for the current user. Use when users want to tune agents, models, prompts, custom agents, skills, MCPs, presets, or plugin behavior. Also use when recurring workflow friction suggests a safe config or prompt improvement.
---

# totempole Configuration Skill
...
- `.totem/` or `~/.config/totem/` plugin/agent configuration
...
totempole is configured through a plugin config file, usually:
...
| Path | Use |
|---|---|
| `~/.config/totem/totem.json` | Totem core config: plugin registration and providers |
| `~/.config/totem/totempole.jsonc` | User plugin config with comments/trailing commas; takes precedence over `.json` |
| `~/.config/totem/totempole.json` | User plugin config generated by installer |
| `<project>/.totem/totempole.json` | Project-local plugin overrides checked before user config |
| `~/.config/totem/totempole/{agent}.md` | Full prompt replacement for a built-in agent |
| `~/.config/totem/totempole/{agent}_append.md` | Append-only prompt tuning for a built-in agent |
| `~/.config/totem/totempole/{preset}/{agent}.md` | Preset-specific full prompt replacement |
| `~/.config/totem/totempole/{preset}/{agent}_append.md` | Preset-specific append-only prompt tuning |
| `~/.config/totem/skills/<skill-name>/SKILL.md` | Installed skill prompt payload |
...
1. If a `preset` is active, check
   `~/.config/totem/totempole/{preset}/` first.
2. Fall back to `~/.config/totem/totempole/`.
...
- Built-in agent `prompt` and `orchestratorPrompt` fields are **not** supported
  in `totempole.json[c]`; use markdown prompt override files instead.
...
```text
~/.config/totem/totempole/orchestrator_append.md
```

or for only the `openai` preset:

```text
~/.config/totem/totempole/openai/orchestrator_append.md
```
...
```text
~/.config/totem/totempole/orchestrator.md
```
...
5. **Tell the user about restart requirements.**
   - Totem may need a restart for config, prompt, agent, skill, MCP, or plugin
     changes to take effect.
   - Phrase it as: "This should apply on the next Totem run; restart Totem
...
1. **Inspect current setup**
   - Read the existing totempole config file.
   - Identify active `preset` and relevant agent blocks.
   - Check whether the user has local project `.totem/` overrides.
...
6. **Explain activation**
   - Tell the user whether the change applies immediately or on next Totem
     run/restart.
...
~/.config/totem/totempole/orchestrator_append.md. Apply it?
```
...
public API changes to it. Apply to your totempole config?
```
...
```text
Updated the config. This should apply on the next Totem run; restart
Totem if you need it immediately.
```
...
- [ ] Did you mention Totem restart/next-run behavior?
