...
- improve their totempole setup based on actual usage using totempole skill;
...
archaeology: analyze historical Totem sessions across all repos to find
...
1. **Load recent sessions** - Query the SQLite database directly:
   ```bash
   bun -e "import Database from 'bun:sqlite'; const db = new Database('/home/mhenke/.local/share/totem/totem.db'); console.log(db.query('SELECT id, directory, title, agent, model, time_created, cost, tokens_input, tokens_output FROM session ORDER BY time_created DESC LIMIT 50').all())"
   ```
...
2. **Load session messages** - For each session ID, query the message table:
   ```bash
   bun -e "import Database from 'bun:sqlite'; const db = new Database('/home/mhenke/.local/share/totem/totem.db'); console.log(db.query('SELECT data FROM message WHERE session_id = ?').all('ses_14de9c68effegtZtlATm42wnz7'))"
   ```
...
```json
{
  ...
  "project": "/home/user/Projects/totempole",
  ...
  "duration_minutes": 18,
  "models_used": ["totem/mimo-v2.5-free"],
...
Store session summaries in `~/.config/totem/totempole/reflections/sessions/`.
...
**Log file issues:**
- Log doesn't exist → "No Totem log found at <path>. Run Totem in at least one repo first."
- Log is empty → "Totem log is empty. No sessions to analyze."
...
- explain restart requirements for Totem config, prompt, agent, skill, MCP, or
  plugin changes.
...
2. Project-local guidance and memories, such as `AGENTS.md`, `.totem/`,
...
3. Existing skills, commands, agents, prompt overrides, MCP permissions, and
   totempole configuration.
4. Recent Totem logs or session artifacts if they are available and safe to
   inspect.
...
- active totempole preset, model routing, skills, and MCP permissions;
...
- When config, prompt, agent, skill, MCP, or plugin files change, tell the user:
  "This should apply on the next Totem run; restart Totem if you need it
  immediately."
