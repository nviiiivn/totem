...
For example, `https://github.com/totem-ai/totem.git` becomes
`totem-ai__totem`. Replace `/` with `__`, strip common `.git` suffixes,
  ...
  "dependencies": [
    {
      ...
      "repoUrl": "https://github.com/totem-ai/totem.git",
      "ref": "v1.3.17",
      "path": ".slim/clonedeps/repos/totem-ai__totem",
...
```gitignore
# BEGIN totempole clonedeps
.slim/clonedeps/repos/
# END totempole clonedeps
```

Update `.ignore` so Totem can read the cloned source while git still ignores
it:
