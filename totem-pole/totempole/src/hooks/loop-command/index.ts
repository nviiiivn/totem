...
function historyDir(): string {
  ...
  return `.totem/loop-history/loop-${timestamp}-${shortID}`;
}
...
export function createLoopCommandHook(): {
  ...
  return {
    registerCommand: (totemConfig) => {
      const cfg = totemConfig.command as Record<string, unknown> | undefined;
      if (cfg?.[COMMAND_NAME]) return;
      if (!totemConfig.command) totemConfig.command = {};
