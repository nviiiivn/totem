...
### Core Abstraction
- **Hook Factory**: `createPhaseReminderHook()` returns an Totem experimental chat message transformer hook
...
1. **Hook Invocation**: Totem calls the `experimental.chat.messages.transform` hook before sending messages to API
...
### Consumers
- **Totem**: Registers the hook via plugin initialization
