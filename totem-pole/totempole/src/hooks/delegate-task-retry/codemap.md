...
Uses the **Observer** pattern via Totem's plugin hook system:
...
### Dependencies
- **Totem Plugin System**: Consumes `tool.execute.after` lifecycle hook
...
### Consumers
- **Totem Core**: Receives enhanced tool output with retry guidance
- **User Workflow**: Displays actionable error context and retry examples in Totem UI
...
```
- Triggered after every tool execution in Totem
- Runs asynchronously but synchronously with respect to tool output processing
- Mutates output object in-place (standard Totem plugin hook contract)
