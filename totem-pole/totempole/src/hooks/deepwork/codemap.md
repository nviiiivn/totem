...
Provides an Totem hook implementation for managing deepwork sessions - heavy, multi-phase coding tasks that require structured planning, phased execution, and continuous validation.
...
- Keep Totem todos synchronized with current phase
...
### Core Abstraction
The hook follows the Totem plugin hook pattern, exposing a factory function `createDeepworkCommandHook()` that returns an object with two methods:

- `registerCommand(config)`: Registers the `deepwork` command in Totem configuration
- `handleCommandExecuteBefore(input, output)`: Intercepts command execution to inject the deepwork activation prompt

### State Management
- Uses Totem's internal agent text part system (`createInternalAgentTextPart`) for output
- Clears existing output parts before injecting deepwork prompt
- Validates task presence before activation

### Integration Points
- Consumes Totem session context (`sessionID`)
- Integrates with Totem command system via `command` configuration
...
1. Plugin initialization calls `registerCommand()` with Totem configuration
...
2. Totem triggers `handleCommandExecuteBefore()` hook
...
3. Agent maintains Totem todo synchronization
...
- **Totem CLI**: Invokes hook when `/deepwork` command is executed
- **Agents** (`@oracle`, `@fixer`, `@explorer`, etc.): Follow deepwork workflow for complex tasks

### Dependencies
- **Totem SDK**: Provides `createInternalAgentTextPart` utility and hook interface
- **Configuration system**: Reads from `totemConfig.command` structure
...
- Maintains synchronization with Totem todos
