...
const cacheMocks = {
  preparePackageUpdate: mock(() => '/tmp/totem'),
  resolveInstallContext: mock(() => ({ installDir: '/tmp/totem' })),
};
...
describe('auto-update-checker/index', () => {
  ...
  test('uses resolved install root for auto-update installs', async () => {
    ...
    expect(getAutoUpdateInstallDir()).toBe('/tmp/totem');
  });
  ...
  test('updates enabled companion after plugin auto-update', async () => {
    ...
    ).toHaveBeenCalledWith('/tmp/totem/node_modules/totempole');
  ...
  test('does not auto-update across major versions', async () => {
    ...
    expect(showToast).toHaveBeenCalledWith({
      body: {
        title: 'totempole v2.0.0 is available.',
        message:
          'It requires Totem background subagents.\nRun: bunx totempole@latest install',
        variant: 'info',
        duration: 12000,
      },
    });
