...
mock.module('../../cli/config-manager', () => ({
  ...
  getTotemConfigPaths: () => [
    '/mock/config/totem.json',
    '/mock/config/totem.jsonc',
  ],
}));
...
describe('auto-update-checker/cache', () => {
  describe('resolveInstallContext', () => {
    test('detects Totem packages install root from runtime package path', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p ===
          '/home/user/.cache/totem/packages/totempole@latest/package.json',
      );
      ...
      const context = resolveInstallContext(
        '/home/user/.cache/totem/packages/totempole@latest/node_modules/totempole/package.json',
      );

      expect(context).toEqual({
        installDir:
          '/home/user/.cache/totem/packages/totempole@latest',
        packageJsonPath:
          '/home/user/.cache/totem/packages/totempole@latest/package.json',
      });
  ...
  describe('preparePackageUpdate', () => {
      ...
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p ===
            '/home/user/.cache/totem/packages/totempole@latest/package.json' ||
          p ===
            '/home/user/.cache/totem/packages/totempole@latest/node_modules/totempole',
      );
    ...
    test('keeps working when dependency is already on target version', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p.endsWith('/.cache/totem/package.json') ||
          p.endsWith('/.cache/totem/node_modules/totempole'),
      );
      ...
      const result = preparePackageUpdate('1.0.1', 'totempole', null);

      expect(result?.endsWith('/.cache/totem')).toBe(true);
