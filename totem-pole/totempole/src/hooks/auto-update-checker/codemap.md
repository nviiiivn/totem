...
Implements a background auto-update system for totempole that:
- Checks for plugin updates when new Totem sessions are created
...
- Synchronizes bundled skills from updated packages to Totem's skills directory
- Provides user notifications via Totem TUI toasts

## Design

### Architecture Pattern: Observer Hook

The folder implements an **Totem plugin hook** that observes the `session.created` event and triggers background update checks. This follows the Observer pattern where:
- The hook subscribes to Totem lifecycle events
...
5. **Timeout Protection**: Uses 60-second timeout for `bun install` to prevent stalling Totem
6. **Atomic Skill Sync**: Uses staging directories with rename for atomic skill synchronization

### Data Flow

```
Totem Session Created
    ↓
...
Action: Run bun install in isolated directory
    ↓
Sync: Bundled skills to Totem config/skills/
    ↓
Sync: Companion update (if enabled)
    ↓
Notify: Success/failure via Totem TUI toast
```
...
2. **Event Subscription**: Hook subscribes to `session.created` Totem event
...
- **Secondary Consumers**: 
  ...
  - Totem TUI - toast notifications

### Dependencies

| Dependency | Purpose |
|------------|---------|
| `@totem-ai/plugin` | Totem plugin SDK types |
| `@totem-ai/sdk` | Totem AI SDK |
...
The system reads from Totem configuration files:
- User config: `~/.config/totem/totem.json`
- User config (JSONC): `~/.config/totem/totem.jsonc`
- Local config: `.totem/totem.json` or `.totem/totem.jsonc` in plugin directory

### Event Integration

- **Event Type**: `session.created`
- **Event Source**: Totem plugin lifecycle
- **Event Properties**: Checks for `parentID` to avoid duplicate checks in child sessions

### Cache Integration

Uses Totem's plugin cache directory:
- Platform-specific: `~/.cache/totem/` (Linux/macOS) or `%LOCALAPPDATA%\totem` (Windows)
- Plugin cache: `node_modules/totempole/`
...
- **Retry on Restart**: Companion updates that fail will retry on next Totem restart
- **Atomic Operations**: Skill sync uses staging + rename for atomic updates
- **Graceful Degradation**: If auto-update fails, shows error toast but doesn't crash Totem
...
- **Timeout Protection**: Prevents network or install operations from stalling Totem
