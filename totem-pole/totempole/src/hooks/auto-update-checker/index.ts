...
/**
 * Creates an Totem hook that checks for plugin updates when a new session is created.
...
async function runBackgroundUpdateCheck(
  ...
  if (installSuccess) {
    ...
    messageLines.push('Restart Totem to apply.');
...
function showMajorUpgradeToast(ctx: PluginInput, version: string): void {
  showToast(
    ctx,
    `totempole v${version} is available.`,
 ...
 * Includes a 60-second timeout to prevent stalling Totem.
...
/**
 * Helper to display a toast notification in the Totem TUI.
 * @param ctx The plugin input context.
