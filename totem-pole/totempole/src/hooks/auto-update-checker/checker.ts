...
/**
 * Generates a list of potential Totem configuration file paths.
 * @param directory The current plugin directory to check for local .totem folders.
 */
function getConfigPaths(directory: string): string[] {
  return [
    path.join(directory, '.totem', 'totem.json'),
...
function getLocalDevPath(directory: string): string | null {
  for (const configPath of getConfigPaths(directory)) {
    try {
      ...
      const config = JSON.parse(stripJsonComments(content)) as TotemConfig;
