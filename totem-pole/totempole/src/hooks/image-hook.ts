...
export function processImageAttachments(args: {
  ...
  // Save images inside the project's .totem/images/ directory.
  // This is within the workspace so the read tool won't require extra permissions.
  const saveDir = join(workDir, '.totem', 'images');
  ...
  const gitignorePath = join(workDir, '.totem', '.gitignore');
  try {
  ...
  for (const { msg, imageParts } of messagesWithImages) {
    ...
    // Save each image to .totem/images/ and collect paths
