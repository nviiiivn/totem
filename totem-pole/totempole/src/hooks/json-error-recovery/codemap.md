...
Provides automatic JSON error detection and recovery for Totem plugin tool execution. This hook monitors tool output for JSON parse errors and injects a standardized error reminder to guide users toward correcting their JSON syntax.
...
- **createJsonErrorRecoveryHook()**: Factory function that returns the Totem plugin hook

### Hook Architecture

The hook implements the Totem plugin's `tool.execute.after` lifecycle hook:
...
8. **Result Return**: Modified output is returned to the Totem plugin system

## Integration

### Consumers

- **Primary Consumer**: Totem plugin system via the `tool.execute.after` lifecycle hook
- **Error Path**: JSON errors in tool arguments are detected and surfaced to users

### Dependencies

- **Totem Plugin SDK**: `@totem-ai/plugin` for PluginInput type definitions
- **Lifecycle Events**: Relies on the `tool.execute.after` event being emitted by Totem
