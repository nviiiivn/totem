...
Implements Totem's patch application hook system with fuzzy matching and automatic rescue strategies. This module intercepts `apply_patch` tool invocations, parses custom patch formats, matches patch chunks to file content using multiple comparison strategies, and rewrites patches to apply cleanly across file moves, renames, and content drift.
...
- **Totem**: Hook integrates with `@totem-ai/plugin` tool execution system
