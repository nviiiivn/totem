...
## Responsibility
Implements a reflection hook that enables Totem to analyze repeated workflow patterns and suggest reusable improvements. This hook provides a `/reflect` command that generates contextual prompts for reviewing recent work and identifying workflow friction points worth improving.

## Design

### Core Components
- **Command Registration**: Dynamically registers the `reflect` command in Totem's configuration system
...
- **Hook Pattern**: Follows Totem's hook system for extending plugin functionality
...
### Dependencies
- **Totem Plugin System**: Uses `registerCommand` and `handleCommandExecuteBefore` hook interfaces
- **Configuration System**: Reads and writes to Totem configuration object
- **Command Lifecycle**: Integrates with Totem's command execution pipeline


### Consumers
- **Totem Core**: Consumed by Totem's plugin system during initialization
- **Users**: Invoked via `/reflect` command in Totem sessions
...
- Totem configuration object: Receives the registered command configuration
