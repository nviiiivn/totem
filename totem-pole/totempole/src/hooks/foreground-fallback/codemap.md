...
## Responsibility
Runtime model fallback system for foreground (interactive) agent sessions. When Totem emits rate-limit signals via `message.updated`, `session.error`, or `session.status` events, this manager:
...
- **Event coverage**: Handles three Totem event types:
...
```
Totem Event (message.updated/session.error/session.status)
    ↓
...
- **Event source**: Totem plugin event system provides `message.updated`, `session.error`, `session.status`, `session.deleted` events

### Dependencies
- **Totem SDK**: `PluginInput['client']` for session management and event handling
- **Utilities**:
