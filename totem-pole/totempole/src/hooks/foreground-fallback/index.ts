 ...
 *
 * When Totem fires a session.error, message.updated, or session.status
...
export class ForegroundFallbackManager {
  ...
  constructor(
    private readonly client: TotemClient,
    /**
  ...
  async handleEvent(rawEvent: unknown): Promise<void> {
    ...
    switch (event.type) {
      case 'message.updated': {
        ...
        if (!sessionID) break;
        // Capture agent name when available (Totem includes it on subagent messages)
      ...
      case 'session.status': {
        ...
        // status type. Totem proxies may emit monthly/weekly/5-hour usage
      ...
      case 'subagent.session.created': {
        // Some builds of Totem include the agent name here.
        const props = event.properties as
      ...
      case 'session.deleted': {
        ...
        // Totem emits two shapes depending on context:
  ...
  private async tryFallback(sessionID: string): Promise<void> {
    ...
    try {
      ...
      // undefined at runtime (Totem violates its own declared type), so
      ...
      // totempole but IS present on the real Totem client at
      // runtime (verified by totem-rate-limit-fallback reference impl).
  ...
  private resolveChain(
    ...
    if (agentName) {
      ...
      // Unknown agent (e.g. Totem built-in "compaction" or "title"
