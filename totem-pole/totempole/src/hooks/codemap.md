...
## Responsibility
Implements Totem lifecycle hooks that transform, process, and manage chat messages and attachments during the plugin's message pipeline. These hooks are invoked by Totem's `experimental.chat.messages.transform` API to modify message content before it reaches models or after responses are generated.

## Design

### Core Architecture
- **Factory Pattern**: Each hook is created via a factory function (e.g., `createApplyPatchHook()`, `createAutoUpdateCheckerHook()`) that returns a hook function matching the Totem hook signature.
...
### Hook Categories
1. **Attachment Processing**: `processImageAttachments` - Extracts image data URLs from messages, saves them to `.totem/images/` directory, and replaces image parts with text references containing file paths.
...
```
1. Totem receives chat messages
...
7. Final messages are returned to Totem
```
...
2. For each user message with images:
   ...
   c. Save image to `.totem/images/[sessionID]/` directory
...
3. Hooks are registered with Totem via `experimental.chat.messages.transform`
4. Totem invokes hooks during message lifecycle
```

## Integration

### Consumers
- **Main Plugin**: `src/index.ts` - registers hooks with Totem during plugin initialization
- **Totem Runtime**: Invokes hooks during `experimental.chat.messages.transform` API calls

### Dependencies
- **Totem SDK**: Type definitions for `MessageWithParts`, `MessageInfo`, and hook signatures
...
- **Workspace Directory**: Images are saved to `.totem/images/` within the project workspace
