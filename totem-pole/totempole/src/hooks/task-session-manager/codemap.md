...
- **index.ts**: Main facade that wires hooks into Totem's lifecycle and coordinates between the job board, pending calls, and task context tracking. Implements the plugin hook interface (`tool.execute.before`, `tool.execute.after`, `experimental.chat.messages.transform`, `event`).
...
- **Main Plugin (`src/index.ts`)**: Wires the task session manager hook into Totem's lifecycle via `createTaskSessionManagerHook()`.
