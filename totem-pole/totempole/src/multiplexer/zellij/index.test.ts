  ...
  crossSpawnMock.mockImplementation((command: string[]) => {
    ...
    if (command.includes('list-tabs')) {
      return createSpawnResult(
        0,
        JSON.stringify([{ name: 'totem-agents', tab_id: 5 }]),
      );
    }
...
describe('ZellijMultiplexer', () => {
  ...
  test('current-tab mode spawns a pane in the parent Totem tab', async () => {
    const { ZellijMultiplexer } = await importFreshZellij();
    ...
    expect(newPaneCommand).toEqual([
      ...
      'sh',
      '-lc',
      "totem attach 'http://localhost:4096' --session 'session-1' --dir '/repo'",
    ]);
  ...
  test('current-tab mode targets the parent Totem tab even when another tab is focused', async () => {
    const { ZellijMultiplexer } = await importFreshZellij();
    const zellij = new ZellijMultiplexer('main-vertical', 60, 'current-tab');
