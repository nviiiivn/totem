...
## Responsibility
Implements a Zellij-based multiplexer adapter that creates and manages terminal panes for sub-agent sessions within Zellij workspaces. Provides pane lifecycle management, session isolation, and graceful shutdown for Totem's multiplexer interface.
...
- Handles two operational modes via `paneMode`:
  - `'agent-tab'` (default): Creates dedicated "totem-agents" tab
  - `'current-tab'`: Creates panes in user's current tab

#### Session Management
- **Pane Creation**: Uses `spawnPane()` to create new panes with Totem attach commands
- **Tab Management**: Ensures "totem-agents" tab exists, tracks tab/pane IDs
...
### Shell Integration
- **Command Construction**: Builds `totem attach` commands with session, server URL, and directory
...
2. First sub-agent session:
   - ensureAgentTab() creates "totem-agents" tab if not exists
   - runInPane() focuses default pane and writes Totem attach command
...
- `buildTotemAttachCommand()` constructs safe shell commands with quoted arguments
...
### State Tracking
- `agentTabId`: Caches the "totem-agents" tab ID after creation
