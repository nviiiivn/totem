 ...
 *
 * The default mode creates a dedicated "totem-agents" tab:
...
export class ZellijMultiplexer implements Multiplexer {
  ...
  private async createPaneInCurrentTab(
    ...
    const totemCmd = buildTotemAttachCommand(
      sessionId,
      serverUrl,
      directory,
    );
  ...
  private async runInPane(
    ...
    try {
      ...
      await crossSpawn(
        [zellij, 'action', 'write-chars', buildShellLaunchCommand(totemCmd)],
        {
          stdout: 'ignore',
          stderr: 'ignore',
        },
  ...
  private async ensureAgentTab(
    ...
    try {
      ...
      const existingTab = await this.findTabByName(zellij, 'totem-agents');
      if (existingTab) {
      ...
      const createProc = crossSpawn(
        [zellij, 'action', 'new-tab', '--name', 'totem-agents'],
      ...
      const newTab = await this.findTabByName(zellij, 'totem-agents');
...
function buildTotemAttachCommand(
  sessionId: string,
  serverUrl: string,
  directory: string,
): string {
