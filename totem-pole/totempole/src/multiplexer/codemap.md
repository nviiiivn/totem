...
Provides a unified abstraction layer for terminal multiplexers (tmux and zellij) to spawn, manage, and close panes for child Totem agent sessions. This enables a "multiplexer-assisted" workflow where each child session runs in its own terminal pane, providing better isolation and resource management compared to traditional background processes.
...
The session manager reacts to Totem session events:
...
```
1. Totem creates child session → emits 'session.created' event
2. MultiplexerSessionManager.onSessionCreated()
   ...
   │  │  ├─ Command: totem attach --session-id <sessionId>
...
- Fetches session statuses from Totem server
...
- **Totem Server**: Provides session lifecycle events and status API
...
- Creates panes with plugin-based Totem integration
