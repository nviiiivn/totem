 ...
 * state file. Source of truth is Totem's session.status events: every
...
export class CompanionManager {
   ...
   * the same Totem session dispose the superseded manager immediately so its
  ...
  private spawnIfAvailable(): void {
    ...
    try {
      const child = spawn(bin, [], {
        ...
        env: {
          ...process.env,
          TEMPPOLE_COMPANION_SESSION_ID: this.id,
          ...(this.config.debug === true
            ? { TEMPPOLE_COMPANION_DEBUG: '1' }
            : {}),
        },
        stdio: 'ignore',
      });
