...
Provides the optional animated companion UI feature that visualizes agent activity (busy/idle states) as animated GIFs. This is a user-facing visual overlay that runs separately from Totem's core orchestration.
...
- **Producer (manager.ts)**: `CompanionManager` class
  - Listens to Totem session lifecycle events (`session.status`, `session.deleted`)
  - Tracks agent activity per session (orchestrator, fixers, etc.)
  - Maintains state in a JSON file at `~/.local/share/totem/storage/totempole/companion-state.json`
...
- Binary path determined by:
  ...
  - Default location: `~/.local/share/totem/storage/totempole/bin/`
...
```
Totem Session → CompanionManager.onSessionStatus() → Updates state → Spawns companion binary
```
...
2. **Agent activity events** (`CompanionManager.onSessionStatus()`)
   - Receives `session.status` events from Totem
...
- **Totem events**: Listens to `session.status` and `session.deleted` lifecycle events
...
- `TEMPPOLE_COMPANION_SESSION_ID`: Set when spawning companion binary
- `TEMPPOLE_COMPANION_DEBUG`: Enables debug mode when config.debug is true

### Storage Locations

- **State file**: `~/.local/share/totem/storage/totempole/companion-state.json`
- **Binary**: `~/.local/share/totem/storage/totempole/bin/totempole-companion[.exe]`
- **Metadata**: `~/.local/share/totem/storage/totempole/bin/totempole-companion.json`

### GitHub Release Artifacts

- Repository: `alvinunreal/totempole`
- Release tag pattern: `companion-v{version}`
- Artifact names: `totempole-companion-v{version}-{target}.{tar.gz|zip}`
