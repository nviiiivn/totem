...
const PROMPTS_DIR_NAME = 'totempole';
...
function loadConfigFromPath(
  ...
  try {
    ...
    try {
      ...
      if (!options?.silent) {
        console.warn(
          `[totempole] Invalid JSON in ${configPath}:`,
          message,
        );
      }
      return null;
    ...
    if (!result.success) {
      ...
      if (!options?.silent) {
        console.warn(`[totempole] Invalid config at ${configPath}:`);
    ...
    if (
      ...
      if (!options?.silent) {
        console.warn(
          `[totempole] Error reading config from ${configPath}:`,
          error.message,
        );
      }
    }
 ...
 *
 * @param basePath - Base path without extension (e.g., /path/to/totempole)
 ...
 * Project config uses <directory>/.totem/totempole.
 *
 * @param directory - Project directory to search for .totem config
 ...
 * 1. User config: $TOTEM_CONFIG_DIR/totempole.jsonc or .json,
 *    or ~/.config/totem/totempole.jsonc or .json (or $XDG_CONFIG_HOME)
 * 2. Project config: <directory>/.totem/totempole.jsonc or .json
 *
...
export function loadPluginConfig(
  ...
  const envPreset = process.env.TEMPPOLE_PRESET;
  if (envPreset) {
  ...
  if (config.preset) {
    ...
    if (preset) {
      ...
      if (!options?.silent) {
        console.warn(`[totempole] ${message}`);
      }
    }
  }
...
export function loadAgentPrompt(
  ...
  if (projectDirectory && presetDirName) {
    searchDirs.push(
      path.join(projectDirectory, '.totem', PROMPTS_DIR_NAME, presetDirName),
    );
  }
  // 2. Project root dir
  if (projectDirectory) {
    searchDirs.push(path.join(projectDirectory, '.totem', PROMPTS_DIR_NAME));
  }
  // 3. User preset dirs
  if (presetDirName) {
  ...
  const readFirstPrompt = (
    ...
    for (const dir of searchDirs) {
      ...
      try {
        ...
        console.warn(
          `[totempole] ${errorPrefix} ${promptPath}:`,
