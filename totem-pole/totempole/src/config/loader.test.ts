...
describe('loadPluginConfig', () => {
  ...
  test('loads project config from .totem directory', () => {
    const projectDir = path.join(tempDir, 'project');
    const projectConfigDir = path.join(projectDir, '.totem');
    fs.mkdirSync(projectConfigDir, { recursive: true });
    fs.writeFileSync(
      path.join(projectConfigDir, 'totempole.json'),
      JSON.stringify({
        agents: {
  ...
  test('respects TOTEM_CONFIG_DIR for user config location', () => {
    const customDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'omc-totem-config-'),
    );
    process.env.TOTEM_CONFIG_DIR = customDir;

    // Write plugin config in the custom directory
    fs.writeFileSync(
      path.join(customDir, 'totempole.json'),
      JSON.stringify({
        agents: { oracle: { model: 'custom/model-from-totem-config-dir' } },
      }),
    );
    ...
    expect(config.agents?.oracle?.model).toBe(
      'custom/model-from-totem-config-dir',
    );

    fs.rmSync(customDir, { recursive: true, force: true });
  });

  test('falls back to default user config dir when TOTEM_CONFIG_DIR has no config', () => {
    const customDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'omc-totem-config-empty-'),
    );
    process.env.TOTEM_CONFIG_DIR = customDir;

    const defaultConfigDir = path.join(userConfigDir, 'totem');
    fs.mkdirSync(defaultConfigDir, { recursive: true });
    fs.writeFileSync(
      path.join(defaultConfigDir, 'totempole.json'),
      JSON.stringify({
...
describe('onWarning callback', () => {
  ...
  test('read error calls onWarning with read-error', () => {
    const projectDir = path.join(tempDir, 'project');
    const projectConfigDir = path.join(projectDir, '.totem');
    const configPath = path.join(projectConfigDir, 'totempole.json');
    fs.mkdirSync(projectConfigDir, { recursive: true });
...
describe('deepMerge behavior', () => {
  ...
  test('merges nested agent configs from user and project', () => {
    // Create user config
    const userTotemDir = path.join(userConfigDir, 'totem');
    fs.mkdirSync(userTotemDir, { recursive: true });
    fs.writeFileSync(
      path.join(userTotemDir, 'totempole.json'),
      JSON.stringify({
        agents: {
...
describe('environment variable preset override', () => {
  ...
  test('Env var overrides preset from config file', () => {
    ...
    process.env.TEMPPOLE_PRESET = 'env-preset';
    const config = loadPluginConfig(projectDir);
  ...
  test('Env var is ignored if undefined', () => {
    ...
    delete process.env.TEMPPOLE_PRESET;
    const config = loadPluginConfig(projectDir);
    expect(config.preset).toBe('config-preset');
    expect(config.agents?.oracle?.model).toBe('config-model');
  });
  ...
  test('Env var with nonexistent preset warns and falls back', () => {
    ...
    process.env.TEMPPOLE_PRESET = 'typo-preset';
    const consoleWarnSpy = spyOn(console, 'warn');
    const config = loadPluginConfig(projectDir);
...
describe('JSONC config support', () => {
  ...
  test('loads .jsonc file with single-line comments', () => {
    ...
    fs.mkdirSync(projectConfigDir, { recursive: true });
    fs.writeFileSync(
      path.join(projectConfigDir, 'totempole.jsonc'),
      `{
        // This is a comment
        "agents": {
  ...
  test('loads user config from .jsonc', () => {
    const userTotemDir = path.join(tempDir, 'user-config', 'totem');
    fs.mkdirSync(userTotemDir, { recursive: true });
    fs.writeFileSync(
      path.join(userTotemDir, 'totempole.jsonc'),
      `{
...
describe('loadAgentPrompt', () => {
  ...
  test('loads replacement prompt from {agent}.md', () => {
    const promptsDir = path.join(tempDir, 'totem', 'totempole');
  ...
  test('respects TOTEM_CONFIG_DIR for prompt location', () => {
    const customDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'omc-prompt-config-'),
    );
    process.env.TOTEM_CONFIG_DIR = customDir;

    const promptsDir = path.join(customDir, 'totempole');
    fs.mkdirSync(promptsDir, { recursive: true });
    fs.writeFileSync(
      path.join(promptsDir, 'oracle.md'),
      'prompt from TOTEM_CONFIG_DIR dir',
    );

    const result = loadAgentPrompt('oracle');
    expect(result.prompt).toBe('prompt from TOTEM_CONFIG_DIR dir');

    fs.rmSync(customDir, { recursive: true, force: true });
  });

  test('falls back to default prompt dir when TOTEM_CONFIG_DIR has no prompt', () => {
    const customDir = fs.mkdtempSync(
