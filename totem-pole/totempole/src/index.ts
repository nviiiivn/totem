import type { Plugin, ToolDefinition } from '@totem-ai/plugin';
...
/**
 * Best-effort log to totem's app logger.
 * Wrapped in try/catch to avoid deadlocking on totem v1.4.8–v1.4.9
...
async function appLog(
  ...
  try {
    await ctx.client.app.log({
      body: { service: 'totempole', level, message },
    });
  } catch {
    ...
    console.error(`[totempole] ${prefix}: ${message}`);
  }
}
...
const totempole: Plugin = async (ctx) => {
  const sessionId = new Date().toISOString().replace(/[-:]/g, '').slice(0, 15);
  initLogger(sessionId);

  if (isPluginDisabledByEnv()) {
    log('[plugin] disabled by TEMPPOLE_DISABLE');
    return {};
  }
  ...
  try {
    ...
    // Totem ever fully re-runs the plugin function (not just the
    ...
    // Initialize MultiplexerSessionManager to handle Totem's built-in
    // Task tool sessions
    ...
    await appLog(
      ctx,
      'error',
      `INIT FAILED: ${String(err)}. Report at github.com/alvinunreal/totempole/issues/310`,
    );
    throw err;
  }
  ...
  if (
    ...
    const msg = [
      ...
      'If you recently updated totem, see:',
      '  github.com/alvinunreal/totempole/issues/310',
    ].join('\n');
    log(`[plugin] WARN: ${msg}`);
  ...
  return {
    ...
    config: async (totemConfig: Record<string, unknown>) => {
      ...
      if (
        ...
        !(totemConfig as { default_agent?: string }).default_agent
      ) {
        (totemConfig as { default_agent?: string }).default_agent =
          'orchestrator';
      }
      ...
      // user-supplied fields (e.g. tools, permission) from totem.json
      if (!totemConfig.agent) {
        totemConfig.agent = { ...agents };
      } else {
        for (const [name, pluginAgent] of Object.entries(agents)) {
          const existing = (totemConfig.agent as Record<string, unknown>)[
            name
          ] as Record<string, unknown> | undefined;
          if (existing) {
            // Shallow merge: plugin defaults first, user overrides win
            (totemConfig.agent as Record<string, unknown>)[name] = {
              ...pluginAgent,
              ...existing,
            };
          } else {
      ...
      const configAgent = totemConfig.agent as Record<string, unknown>;
      ...
      if (Object.keys(modelArrayMap).length > 0) {
        for (const [agentName, models] of Object.entries(modelArrayMap)) {
          ...
          // require entries in totemConfig.provider - some are loaded
          // automatically by totem (e.g. github-copilot, openrouter).
          ...
          if (entry) {
            ...
            // Agent exists in slim but not in totemConfig.agent -
            // create entry
      ...
      // Merge MCP configs
      const configMcp = totemConfig.mcp as
        | Record<string, unknown>
        | undefined;
      if (!configMcp) {
        totemConfig.mcp = { ...mcps };
      } else {
        Object.assign(configMcp, mcps);
      }

      // Get all MCP names from the merged config (built-in + custom)
      const mergedMcpConfig = totemConfig.mcp as
        | Record<string, unknown>
        | undefined;
      ...
      interviewManager.registerCommand(totemConfig);
      deepworkCommandHook.registerCommand(totemConfig);
      reflectCommandHook.registerCommand(totemConfig);
      loopCommandHook.registerCommand(totemConfig);
      presetManager.registerCommand(totemConfig);
    },
    ...
    event: async (input) => {
      ...
      // Handle multiplexer pane spawning for Totem's Task tool sessions
    ...
    'experimental.chat.system.transform': async (
      ...
      // them back into one element so Totem emits a single system
      // message.
