...
describe('runDoctorCheck', () => {
  ...
  test('valid project config returns ok', () => {
    const projectDir = path.join(tempDir, 'project');
    const configDir = path.join(projectDir, '.totem');
    fs.mkdirSync(configDir, { recursive: true });
    fs.writeFileSync(
      path.join(configDir, 'totempole.jsonc'),
      `{
  ...
  test('invalid JSON returns not ok with invalid-json error', () => {
    ...
    fs.mkdirSync(configDir, { recursive: true });
    fs.writeFileSync(
      path.join(configDir, 'totempole.json'),
      '{ invalid json }',
    );
  ...
  test('config deleted after discovery returns read error with path', () => {
    const projectDir = path.join(tempDir, 'project');
    const configDir = path.join(projectDir, '.totem');
    const configPath = path.join(configDir, 'totempole.json');
    fs.mkdirSync(configDir, { recursive: true });
  ...
  test('empty config file returns invalid-json error', () => {
    ...
    const configDir = path.join(projectDir, '.totem');
    fs.mkdirSync(configDir, { recursive: true });
    fs.writeFileSync(path.join(configDir, 'totempole.json'), '');
