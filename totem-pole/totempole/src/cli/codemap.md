...
CLI entry point and command-line interface for the totempole plugin. Provides installation, configuration, and diagnostic commands for setting up and managing the Totem plugin.

## Design

The CLI follows a command pattern with two primary commands:
- `install`: Sets up the plugin with Totem (adds plugin to config, configures background subagents, installs companion, writes configuration)
...
- **background-subagents.ts**: Shell integration for Totem background subagents (persistent agent processes)
...
```
1. CLI invoked (bunx totempole install/doctor)
...
2. Check Totem installation
3. Add plugin to Totem configuration (totem.json/totem.jsonc)
4. Add TUI version badge (tui.json/tui.jsonc)
5. Warm Totem plugin cache (for package manager installations)
6. Disable Totem default agents (explore, general)
...
10. Write totempole configuration (totempole.json)
...
1. Determine config directory:
   - TOTEM_CONFIG_DIR environment variable (highest priority)
   - XDG_CONFIG_HOME/totem
   - ~/.config/totem (default)
2. Resolve file paths:
   - totem.json → totem.jsonc → fallback to totem.json
   - totempole.json → totempole.jsonc → fallback
...
2. Determine target file:
   ...
   - fish: ~/.config/fish/conf.d/totem-background-subagents.fish
3. Write environment variable export:
   - export TOTEM_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true
...
- **Totem**: CLI commands are invoked by Totem's plugin system
...
### Integration Points
- **Totem plugin system**: CLI commands integrate via Totem's command execution
- **Shell environment**: Background subagents modify shell startup files
- **Configuration files**: Atomic writes to user config directory (~/.config/totem/)
...
|------|---------|------------|
| totem.json/totem.jsonc | Totem main config | config-io.ts |
| tui.json/tui.jsonc | Totem TUI config | config-io.ts |
| totempole.json | Plugin-specific config | providers.ts |

## Commands

### `install` Command
Sets up totempole plugin with Totem.

**Usage:**
```bash
bunx totempole install [OPTIONS]
```
...
**Available presets:** openai, totem-go, kimi, copilot, zai-plan
