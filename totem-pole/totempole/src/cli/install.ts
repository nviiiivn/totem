...
const GITHUB_REPO = 'alvinunreal/totempole';
const GITHUB_URL = `https://github.com/${GITHUB_REPO}`;

function printHeader(isUpdate: boolean): void {
  console.log();
  console.log(
    `${BOLD}totempole ${isUpdate ? 'Update' : 'Install'}${RESET}`,
  );
...
async function checkTotemInstalled(): Promise<{
  ok: boolean;
  version?: string;
  path?: string;
}> {
  const installed = await isTotemInstalled();
  if (!installed) {
    printError('Totem is not installed on this system.');
    printInfo('Install it with:');
    console.log(
      `     ${BLUE}curl -fsSL https://totem.ai/install | bash${RESET}`,
    );
    console.log();
    printInfo('Or if already installed, add it to your PATH:');
    console.log(`     ${BLUE}export PATH="$HOME/.local/bin:$PATH"${RESET}`);
    console.log(`     ${BLUE}export PATH="$HOME/.totem/bin:$PATH"${RESET}`);
    return { ok: false };
  }
  const version = await getTotemVersion();
  const path = getTotemPath();
  const detectedVersion = version ?? '';
  const pathInfo = path ? ` (${DIM}${path}${RESET})` : '';
  printSuccess(`Totem ${detectedVersion} detected${pathInfo}`);
...
export async function configureBackgroundSubagents(
  ...
  if (
    isBackgroundSubagentsEnabled(
      process.env.TOTEM_EXPERIMENTAL_BACKGROUND_SUBAGENTS,
    )
  ) {
    printSuccess(
      'Totem background subagents already enabled in environment',
    );
  ...
  if (config.backgroundSubagents === 'no') {
    printInfo('Totem background subagents shell setup skipped.');
  ...
  if (config.backgroundSubagents === 'ask') {
    ...
    console.log();
    printInfo(
      'V2 requires Totem background subagents for default orchestration.',
    );
    printInfo(
      `The installer can add the required environment export to ${target}.`,
    );
    const shouldWrite = await confirm(
      'Add TOTEM_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true now?',
      true,
    );
    if (!shouldWrite) {
...
async function runInstall(config: InstallConfig): Promise<number> {
  ...
  printStep(step++, totalSteps, 'Checking Totem installation...');
  if (config.dryRun) {
    printInfo('Dry run mode - skipping Totem check');
  } else {
    const { ok } = await checkTotemInstalled();
    if (!ok) return 1;
  }
  printStep(step++, totalSteps, 'Adding totempole plugin...');
  if (config.dryRun) {
    printInfo('Dry run mode - skipping plugin installation');
  } else {
    const pluginResult = await addPluginToTotemConfig();
  ...
  if (config.dryRun) {
    ...
    const tuiResult = await addPluginToTotemTuiConfig();
  ...
  printStep(step++, totalSteps, 'Warming Totem plugin cache...');
  if (config.dryRun) {
    printInfo('Dry run mode - skipping cache warm-up');
  } else {
    const cacheResult = await warmTotemPluginCache();
    if (cacheResult === null) {
      ...
      handleStepResult(cacheResult, 'Totem cache warmed');
    }
  }

  printStep(step++, totalSteps, 'Disabling Totem default agents...');
  if (config.dryRun) {
  ...
  printStep(step++, totalSteps, 'Enabling Totem LSP integration...');
  if (config.dryRun) {
  ...
  printStep(step++, totalSteps, 'Configuring Totem background subagents...');
  ...
  printStep(step++, totalSteps, 'Writing totempole configuration...');
  if (config.dryRun) {
  ...
  console.log(`     ${BLUE}$ totem auth login${RESET}`);
  console.log();
  console.log('  2. Refresh the models Totem can see:');
  console.log(`     ${BLUE}$ totem models --refresh${RESET}`);
  console.log();
  console.log('  3. Review your generated config:');
  console.log(`     ${BLUE}${configPath}${RESET}`);
  console.log();
  console.log('  4. Start Totem:');
  if (backgroundSubagents.enabledNow) {
    console.log(`     ${BLUE}$ totem${RESET}`);
    ...
    console.log(`     ${BLUE}$ totem${RESET}`);
    console.log(
      `     ${DIM}Or restart your terminal before running totem.${RESET}`,
    );
  } else {
    console.log(
      `     ${BLUE}$ TOTEM_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true totem${RESET}`,
    );
  }
  console.log();
  ...
  const modelsInfo =
    config.preset && config.preset !== 'openai'
      ? `Generated OpenAI and Totem Go presets; ${config.preset} is active.`
      : 'Generated OpenAI and Totem Go presets; OpenAI is active by default.';
