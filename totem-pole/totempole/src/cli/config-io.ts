...
const PACKAGE_NAME = 'totempole';
const DEFAULT_TOTEM_AGENTS_TO_DISABLE = ['explore', 'general'] as const;
...
function getPlugins(config: TotemConfig): unknown[] {
  return Array.isArray(config.plugin) ? config.plugin : [];
}

function getPluginEntries(config: TotemConfig): string[] {
...
/**
 * Reads the Totem config to find the pinned version for this plugin.
...
function getTotemPluginCacheDir(version?: string): string {
  const cacheDir =
    process.env.XDG_CACHE_HOME?.trim() || join(homedir(), '.cache');
  const suffix = version
    ? `${PACKAGE_NAME}@${version}`
    : `${PACKAGE_NAME}@latest`;
  return join(cacheDir, 'totem', 'packages', suffix);
}

function writeTotemPluginCacheManifest(
  cacheDir: string,
...
function removeTotemPluginCacheArtifacts(cacheDir: string): void {
...
function verifyTotemPluginCache(cacheDir: string): ConfigMergeResult | null {
...
export async function warmTotemPluginCache(): Promise<ConfigMergeResult | null> {
  const cliEntryPath = process.argv[1];
  if (!cliEntryPath) {
    return null;
  }
  ...
  const cacheDir = getTotemPluginCacheDir(cacheVersion);
  ...
  try {
    ...
    return {
      success: false,
      configPath: cacheDir,
      error: `Failed to create Totem cache directory: ${err}`,
    };
  }

  const manifestError = writeTotemPluginCacheManifest(
    cacheDir,
    cacheVersion,
  );
  if (manifestError) return manifestError;

  removeTotemPluginCacheArtifacts(cacheDir);
  ...
  try {
    ...
    const verificationError = verifyTotemPluginCache(cacheDir);
    ...
    return {
      success: false,
      configPath: cacheDir,
      error: `Failed to warm Totem cache: ${err}`,
    };
  }
}
...
export function parseConfigFile(path: string): {
  ...
  try {
    ...
    return { config: JSON.parse(stripJsonComments(content)) as TotemConfig };
  } catch (err) {
 ...
 */
export function writeConfig(configPath: string, config: TotemConfig): void {
...
export async function addPluginToTotemConfig(): Promise<ConfigMergeResult> {
  ...
  try {
    ...
    // Remove existing totempole entries
    ...
    return {
      success: false,
      configPath,
      error: `Failed to update totem config: ${err}`,
    };
  }
}

export async function addPluginToTotemTuiConfig(): Promise<ConfigMergeResult> {
  ...
  try {
    ...
    return {
      success: false,
      configPath,
      error: `Failed to update totem TUI config: ${err}`,
    };
  }
}

// Removed: addAuthPlugins - no longer needed with cliproxy
// Removed: addProviderConfig - default totem now has kimi provider config
...
export function disableDefaultAgents(): ConfigMergeResult {
  ...
  try {
    ...
    for (const agentName of DEFAULT_TOTEM_AGENTS_TO_DISABLE) {
...
export function canModifyTotemConfig(): boolean {
  try {
    const configPath = getExistingConfigPath();
...
export function detectCurrentConfig(): DetectedConfig {
  ...
  result.hasAntigravity = plugins.some((p) =>
    p.startsWith('totem-antigravity-auth'),
  );
  ...
  if (liteConfig && typeof liteConfig === 'object') {
    ...
    if (agents && typeof agents === 'object') {
      ...
      result.hasTotemZen ||= models.some((m) => m.startsWith('totem/'));
