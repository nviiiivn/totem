...
Options:
  ...
  --background-subagents=ask|yes|no
                          Persist required Totem background subagent env
...
The installer generates OpenAI and Totem Go presets by default.
OpenAI is active unless --preset selects another generated preset.
For the full config reference, see docs/configuration.md.

Examples:
  bunx totempole install
  bunx totempole install --no-tui --skills=yes
  bunx totempole install --background-subagents=yes
  bunx totempole install --preset=totem-go
  bunx totempole install --reset
  bunx totempole doctor
`);
}
