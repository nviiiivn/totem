...
describe('providers', () => {
  ...
  test('generateLiteConfig defaults to openai and includes generated presets', () => {
    ...
    expect((config.presets as any)['totem-go']).toBeDefined();
    expect((config.presets as any)['totem-go'].observer.model).toBe(
      'totem-go/kimi-k2.6',
    );
  ...
  test('generateLiteConfig can set totem-go as active preset', () => {
    const config = generateLiteConfig({
      hasTmux: false,
      installCustomSkills: false,
      preset: 'totem-go',
    ...
    expect(config.preset).toBe('totem-go');
    expect(config.disabled_agents).toEqual([]);
    expect((config.presets as any).openai).toBeDefined();
    const agents = (config.presets as any)['totem-go'];
    expect(agents).toBeDefined();
    expect(agents.orchestrator.model).toBe('totem-go/glm-5.2');
    expect(agents.oracle.model).toBe('totem-go/qwen3.7-max');
    expect(agents.oracle.variant).toBe('max');
    expect(agents.council).toBeUndefined();
    expect(agents.librarian.model).toBe('totem-go/deepseek-v4-flash');
    expect(agents.explorer.model).toBe('totem-go/deepseek-v4-flash');
    expect(agents.designer.model).toBe('totem-go/kimi-k2.7-code');
    expect(agents.fixer.model).toBe('totem-go/deepseek-v4-flash');
    expect(agents.fixer.variant).toBe('high');
    expect(agents.observer.model).toBe('totem-go/kimi-k2.6');
  });
