...
const ENV_NAME = 'TOTEM_EXPERIMENTAL_BACKGROUND_SUBAGENTS';
const START_MARKER = '# >>> totempole background subagents >>>';
const END_MARKER = '# <<< totempole background subagents <<<';
...
export function detectBackgroundSubagentsTarget(
  ...
  if (shell === 'fish') {
    ...
    return join(
      ...
      'fish',
      'conf.d',
      'totem-background-subagents.fish',
    );
  }
  return undefined;
}
...
export function manualBackgroundSubagentsInstructions(options?: {
  ...
  if (shell === 'fish') {
    return `Start Totem with background subagents enabled:\n  env ${ENV_NAME}=true totem\n\nOr add this to your fish startup file:\n  ${fishSnippet}`;
  }
