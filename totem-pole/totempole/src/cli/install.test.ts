...
const originalIsTotemInstalled = actualConfigManager.isTotemInstalled;
const originalGetTotemVersion = actualConfigManager.getTotemVersion;
const originalGetTotemPath = actualConfigManager.getTotemPath;
const originalAddPluginToTotemConfig =
  actualConfigManager.addPluginToTotemConfig;
const originalAddPluginToTotemTuiConfig =
  actualConfigManager.addPluginToTotemTuiConfig;
const originalWarmTotemPluginCache =
  actualConfigManager.warmTotemPluginCache;
...
mock.module('./config-manager', () => {
  return {
    ...actualConfigManager,
    isTotemInstalled: async () =>
      enableInstallMocks ? true : originalIsTotemInstalled(),
    getTotemVersion: async () =>
      enableInstallMocks ? '1.0.0' : originalGetTotemVersion(),
    getTotemPath: () =>
      enableInstallMocks
        ? '/usr/local/bin/totem'
        : originalGetTotemPath(),
    addPluginToTotemConfig: async () =>
      enableInstallMocks
        ? { success: true, configPath: '/path' }
        : originalAddPluginToTotemConfig(),
    addPluginToTotemTuiConfig: async () =>
      enableInstallMocks
        ? { success: true, configPath: '/path' }
        : originalAddPluginToTotemTuiConfig(),
    warmTotemPluginCache: async () =>
      enableInstallMocks
        ? { success: true, configPath: '/path' }
        : originalWarmTotemPluginCache(),
