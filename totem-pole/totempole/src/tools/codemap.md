...
Centralized tool factory and registry for the Totem plugin system. This directory defines all executable tools exposed to Totem agents, including:
...
These tools enable agents to perform file operations, orchestrate multi-model consensus, manage background tasks, and interact with external systems while maintaining security boundaries through the Totem tool schema.

## Design

### Architecture Pattern: **Tool Factory Pattern**

Each tool is implemented as a factory function that returns a `ToolDefinition` record compatible with the `@totem-ai/plugin` SDK. The pattern provides:
...
2. Tool Factory Execution
   ...
   └─> Registers tool with Totem via plugin API
...
3. Tool Invocation
   ...
   └─> Totem presents result to agent
```
...
- **Main Plugin** (`src/index.ts`):
  - `registerTools()` - Registers all exported tools with Totem
...
| Dependency | Purpose |
|------------|---------|
| `@totem-ai/plugin` | Tool schema and execution framework |
...
- **Presets**: Defined in plugin config (`totempole.jsonc`), managed by `preset-manager.ts`
...
- Requires council agent to be registered in Totem
...
- **Integration Tests**: Tools tested with mock dependencies and Totem context
