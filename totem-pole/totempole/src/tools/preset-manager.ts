...
import type { PluginInput } from '@totem-ai/plugin';
 ...
 * plugin-facing TUI snapshot. It deliberately does not call Totem's
 * client.config.update() or instance.dispose() from command hooks because
 * Totem continues the same command into the prompt loop after
...
export function createPresetManager(ctx: PluginInput, config: PluginConfig) {
  ...
  async function handleCommandExecuteBefore(
    ...
    // Clear the template so Totem doesn't send it to the LLM
  ...
  /**
   * Register the /preset command in the Totem config.
   */
  function registerCommand(totemConfig: Record<string, unknown>): void {
    const configCommand = totemConfig.command as
      | Record<string, unknown>
      | undefined;
    if (!configCommand?.[COMMAND_NAME]) {
      if (!totemConfig.command) {
        totemConfig.command = {};
      }
      (totemConfig.command as Record<string, unknown>)[COMMAND_NAME] = {
   ...
   * active Totem instance. The preset is applied by the plugin config path
   * when Totem reloads the plugin in a safe lifecycle boundary.
   */
  async function switchPreset(
    ...
    if (!preset) {
      ...
      const hint =
        available.length > 0
          ...
          : 'No presets configured. Define presets in totempole.jsonc.';
      output.parts.push(
        createInternalAgentTextPart(
    ...
    output.parts.push(
      createInternalAgentTextPart(
        `Saved preset "${presetName}". Restart or reload Totem to apply it to agent configuration. The current session was not reloaded to avoid interrupting the active conversation.\n${summaryParts.join('\n')}`,
      ),
    );
  }
  ...
  function formatPresetList(presets: Record<string, Preset>): string {
    ...
    if (names.length === 0) {
      return 'No presets configured. Define presets in totempole.jsonc under the "presets" field.';
    }
