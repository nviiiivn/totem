...
type TotemClient = PluginInput['client'];
...
function findPreferredTotemConfigPath(baseDir: string) {
  for (const file of ['totem.jsonc', 'totem.json']) {
...
async function readTotemConfigFile(configPath: string | undefined) {
...
async function readEffectiveTotemConfig(directory: string) {
  const projectDir = path.join(directory, '.totem');
  const userDirs = getConfigSearchDirs();
  const projectPath = findPreferredTotemConfigPath(projectDir);
  const userPath = userDirs
    .map((configDir) => findPreferredTotemConfigPath(configDir))
    .find(Boolean);

  const userConfig = await readTotemConfigFile(userPath);
  const projectConfig = await readTotemConfigFile(projectPath);
...
export async function readSecondaryModelFromConfig(directory: string) {
  try {
    ...
    const totemConfig = await readEffectiveTotemConfig(directory);
    pushModel(
      typeof totemConfig.small_model === 'string'
        ? totemConfig.small_model
        : undefined,
    );
 ...
 * whenever the delete failed (e.g. during an Totem instance dispose/reload
