...
- `secondary-model.ts` depends on the Totem plugin client (`PluginInput['client']`) to spawn an isolated helper session, resolve `small_model` from the effective Totem config, and resolve `explorer` / `librarian` fallbacks from slim's own plugin config loader.
