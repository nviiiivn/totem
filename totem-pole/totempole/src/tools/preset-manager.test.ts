...
let previousTotemConfigDir: string | undefined;
let tempDir: string;

beforeEach(() => {
  previousXdgDataHome = process.env.XDG_DATA_HOME;
  previousXdgConfigHome = process.env.XDG_CONFIG_HOME;
  previousTotemConfigDir = process.env.TOTEM_CONFIG_DIR;
...
afterEach(() => {
  ...
  if (previousTotemConfigDir === undefined) {
    delete process.env.TOTEM_CONFIG_DIR;
  } else {
    process.env.TOTEM_CONFIG_DIR = previousTotemConfigDir;
  }
...
describe('createPresetManager', () => {
  describe('handleCommandExecuteBefore', () => {
    ...
    test('switches preset state without config.update or instance.dispose', async () => {
      ...
      expect(text).toContain('Restart or reload Totem');
    ...
    test('persists preset changes from JSONC user config', async () => {
      const configDir = path.join(tempDir, 'totem-config');
      fs.mkdirSync(configDir, { recursive: true });
      process.env.TOTEM_CONFIG_DIR = configDir;

      const configPath = path.join(configDir, 'totempole.jsonc');
      fs.writeFileSync(
        configPath,
        `{
  ...
  describe('registerCommand', () => {
    test('registers preset command when not present', () => {
      ...
      const config: PluginConfig = {};
      const manager = createPresetManager(ctx, config);
      const totemConfig: Record<string, unknown> = {};

      manager.registerCommand(totemConfig);

      const command = (totemConfig.command as Record<string, unknown>)
    ...
    test('does not overwrite existing preset command', () => {
      ...
      expect((totemConfig.command as Record<string, unknown>).preset).toBe(
        existing,
      );
    });
  });
