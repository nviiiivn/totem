...
Defines Model Context Protocol (MCP) server configurations and integrations for the Totem plugin. This module provides built-in MCP servers for web search, code search, and documentation lookup, enabling agents to access external tools and resources via the MCP standard.
...
- **Primary consumer**: `src/index.ts` - Main plugin entry point
  ...
  - Receives McpConfig record and registers MCPs with Totem
...
```typescript
// In user configuration (e.g., ~/.config/totem/totempole.json)
{
  "mcp": {
    "disabled": ["websearch"],
    "websearch": {
...
- **Integration tests**: MCP servers are tested via integration with Totem's MCP runtime
