...
Centralized utilities and shared abstractions used across the totempole plugin. This folder provides:
...
- **Logger** (`logger.ts`): File-based logging with 7-day retention, automatic directory creation, and write queuing. Logs are written to `~/.local/share/totem/log/totempole.<sessionId>.log` and cleaned up on initialization.
...
2. Logs are appended to `~/.local/share/totem/log/totempole.<sessionId>.log`
...
- **@totem-ai/sdk**: PluginInput type for session utilities
