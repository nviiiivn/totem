...
Core plugin implementation for **totempole**, providing:
- Main plugin initialization and Totem integration (`index.ts`)
...
- **Plugin Pattern**: The plugin follows Totem's plugin architecture with a single exported plugin function that returns agent, tool, and MCP registrations
- **Facade Pattern**: `index.ts` acts as a facade that composes multiple subsystems (agents, tools, MCPs, hooks, multiplexer)
- **Observer Pattern**: Event-driven architecture using Totem's event system for session lifecycle, message updates, and tool execution
...
```
Totem Core → Plugin Initialization (index.ts)
...
1. **Plugin Registration**: TUI plugin registered with Totem's TUI system via `tui` export
...
1. **State Path Resolution**: Determines XDG-compliant state directory (`~/.local/share/totem/storage/totempole/tui-state.json`)
...
- **Totem Core**: Main plugin entry point consumed by Totem's plugin system
- **TUI System**: `tui.ts` slot registration consumed by Totem's TUI renderer
- **Agents**: Agent configurations consumed by Totem's agent registry
- **Tools**: Tool definitions consumed by Totem's tool system
- **MCPs**: MCP definitions consumed by Totem's MCP registry
...
4. **Event Propagation**: Events flow from Totem → plugin handlers → subsystems → state updates

### Configuration Integration

- Plugin config loaded via `loadPluginConfig()` with support for:
  - User overrides from `~/.config/totem/totempole.json`
  - Runtime presets via `/preset` command
  - Environment-based disablement via `TEMPPOLE_DISABLE`
- Agent configurations merged with user settings from Totem config
...
- **Event-Driven**: Minimal polling; relies on Totem's event system
