> **Vendored third-party plugin.** Everything below this notice is [alvinunreal/oh-my-opencode-slim](https://github.com/alvinunreal/oh-my-opencode-slim) — not written by, or affiliated with, this fork. All links and stargazer counts point at the real upstream project.
...
インストール時に Totem Go を有効にするには、`bunx oh-my-opencode-slim@latest install --preset=totem-go` を実行するか、インストール後に `~/.config/totem/totempole.json` のデフォルトプリセット名を変更してください。
