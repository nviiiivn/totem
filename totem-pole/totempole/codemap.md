# Repository Atlas: totempole

## Project Responsibility

`totempole` is an Totem plugin that implements a specialist-agent operating model on top of the host runtime. Its core responsibilities include:
...
This codemap covers the plugin repository itself and excludes the nested `totem/` upstream checkout.
...
| `scripts/generate-schema.ts` | Generates `totempole.schema.json` from the Zod config schema. |
...
| `src/skills/` | Bundled install-time Totem skills shipped as static payloads. | [View Map](src/skills/codemap.md) |
...
2. **Interactive request handling**
   ...
   - Tool calls resolve through `src/tools/` or built-in Totem tools.
...
3. **Delegated execution**
   - Native Totem background tasks are parsed from `task` output and injected completion messages and tracked in the shared background job board.
...
4. **Install/release path**
   - `src/cli/` configures host Totem instances.
