...
```
Install and configure totempole: https://raw.githubusercontent.com/alvinunreal/oh-my-opencode-slim/refs/heads/master/README.md
```
...
```bash
git clone https://github.com/alvinunreal/oh-my-opencode-slim.git ~/repos/totempole
cd ~/repos/totempole
bun install
bun run build
...
To make Totem Go active during install, run `bunx oh-my-opencode-slim@latest install --preset=totem-go` or change the default preset name in `~/.config/totem/totempole.json` after installation.
...
```jsonc
{
  "$schema": "https://unpkg.com/oh-my-opencode-slim@latest/totempole.schema.json",
  "preset": "openai",
  "presets": {
    "openai": {
