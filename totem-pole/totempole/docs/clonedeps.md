...
6. Updates ignore files so git ignores clone contents while Totem can read
   them.
...
```text
Use clonedeps for the Totem SDK so you can inspect its plugin API internals.
```
...
name. For example, `https://github.com/totem-ai/totem.git` becomes
`.slim/clonedeps/repos/totem-ai__totem/`.
...
- `.slim/clonedeps/repos/totem-ai__totem/` - `totem-ai/totem` at
  `v1.3.17`; inspect `packages/sdk/js` for Totem SDK internals.
```
