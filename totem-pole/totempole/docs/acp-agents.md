...
6. Returns the external agent's final output to Totem.
...
Add `acpAgents` to `~/.config/totem/totempole.jsonc` or a
project-local `.totem/totempole.jsonc` file:
...
Restart Totem after changing config. Then call the generated agent directly:
...
| `description` | string | generated | Role text shown to Totem and the orchestrator. |
...
| `wrapperModel` | string | fixer default | Cheap Totem model used by the wrapper. |
...
- **Agent not available:** restart Totem after editing config.
