...
Complete reference for all configuration files and options in totempole. For repository-specific configurations, custom agents, and prompt directory lookups, see the [Project-local Customization Guide](project-local-customization.md).
...
| File | Purpose |
|------|---------|
| `~/.config/totem/totem.json` | Totem core settings (plugin registration, providers) |
| `~/.config/totem/totempole.json` | Plugin settings - agents, multiplexer, MCPs, council |
| `~/.config/totem/totempole.jsonc` | Same, but with JSONC (comments + trailing commas). Takes precedence over `.json` if both exist |
| `.totem/totempole.json` | Project-local overrides (optional, higher precedence than user config) |
...
Set `TOTEM_CONFIG_DIR` to use a custom user config directory instead of
`~/.config/totem`; install and runtime config discovery both honor it.

Set `TEMPPOLE_DISABLE` to `1`, `true`, `yes`, or `on` to make
totempole return during startup without registering agents, tools,
MCPs, hooks, Companion, or the TUI sidebar. This is a temporary escape hatch:

```bash
TEMPPOLE_DISABLE=1 totem
```

If OmO-slim detects an invalid plugin config for the current project, the TUI sidebar shows a warning. Run `totempole doctor` from your project root for full diagnostics.

---

## Prompt Overriding

Customize agent prompts without modifying source code. Create markdown files in `~/.config/totem/totempole/`:
...
| `acpAgents.<name>.description` | string | - | Description shown to Totem and injected into the orchestrator routing prompt |
...
| `acpAgents.<name>.wrapperModel` | string | fixer default | Cheap Totem model used by the wrapper subagent that calls `acp_run` |
...
| `multiplexer.zellij_pane_mode` | string | `"agent-tab"` | Zellij pane placement: `agent-tab` creates/reuses a dedicated `totem-agents` tab; `current-tab` opens subagents as panes in the tab containing the parent Totem pane, falling back to the focused tab if the parent pane cannot be resolved |
...
> `app-id`/title (`totempole-companion`), for example:
>
> ```kdl
> window-rule {
>     match app-id=r"^totempole-companion$"
>     match title=r"^totempole-companion$"
>     open-floating true
>     open-focused false
...
Totem subagents. The plugin creates a lightweight wrapper agent for each
...
> Pinned plugin entries in `totem.json` (for example
> `"totempole@1.0.1"`) are the true version lock. Those stay pinned
...
Once installed, configure it in your `totempole` settings:
