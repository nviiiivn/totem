...
You can enable the companion by adding a `companion` section to your setting configuration file (`~/.config/totem/totempole.json` or `.totem/totempole.json`):

```jsonc
{
  "companion": {
    "enabled": true,
    "binaryPath": "/path/to/totempole-companion",
...
- **`companion.debug`**: set to `true` to enable verbose native companion debug
  ...
  `$XDG_DATA_HOME/totem/log/` or `~/.local/share/totem/log/`.
...
```bash
bunx totempole install --companion=yes
```
...
Wayland app-id/title: `totempole-companion`.
...
```text
$XDG_DATA_HOME/totem/storage/totempole/bin/totempole-companion
```

If `XDG_DATA_HOME` is unset, this resolves to:

```text
~/.local/share/totem/storage/totempole/bin/totempole-companion
```
...
so Totem startup is not blocked by a slow network. Plugin auto-update also
...
on the next Totem restart. The updater uses a lock to prevent concurrent
Totem processes from replacing the same binary, and stale locks from crashed
...
```text
totempole-companion-v0.1.3-aarch64-apple-darwin.tar.gz
totempole-companion-v0.1.3-x86_64-apple-darwin.tar.gz
totempole-companion-v0.1.3-x86_64-unknown-linux-gnu.tar.gz
totempole-companion-v0.1.3-aarch64-unknown-linux-gnu.tar.gz
totempole-companion-v0.1.3-x86_64-pc-windows-msvc.zip
```
...
```bash
bunx totempole@beta install --companion=yes
```
