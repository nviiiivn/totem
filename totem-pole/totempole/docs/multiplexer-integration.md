...
Use tmux, Zellij, or Herdr to watch subagents work in live panes while Totem keeps running in your main session.
...
When Totem launches child agent sessions, totempole can open panes for those sessions automatically.
...
*Totem running in tmux with live subagent panes.*

> ⚠️ **Current workaround:** Start Totem with `--port` to enable multiplexer integration. The port must match the `TOTEM_PORT` environment variable. This is required until [totem#9099](https://github.com/anomalyco/totem/issues/9099) is resolved.

If you open multiple Totem sessions, use a random high port for each launch instead of hard-coding `4096`.
...
Edit `~/.config/totem/totempole.json` (or `.jsonc`):
...
### 2. Start Totem inside tmux, Zellij, or Herdr
...
Ask Totem to do something that launches subagents. New panes should appear automatically.
...
| `zellij_pane_mode` | string | `"agent-tab"` | Zellij pane placement: `"agent-tab"` creates/reuses a dedicated tab; `"current-tab"` opens panes in the tab containing the parent Totem pane |
...
| **Zellij** | ✅ Supported | Creates a dedicated `totem-agents` tab by default; can open panes in the parent Totem tab with `zellij_pane_mode: "current-tab"`; maps `main-*` layouts to pane directions |
...
**Example: open Zellij subagents in the parent Totem tab**
...
Totem pane, even if another Zellij tab is focused when a subagent starts.
