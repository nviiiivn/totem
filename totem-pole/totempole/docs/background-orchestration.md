...
totempole. It assumes native Totem background subagents are
...
Background orchestration requires an Totem release that includes native
...
| hook-driven completion | Totem injects terminal background task results automatically |
...
one-shot export above before starting Totem.

Use an Totem release that includes native background subagents and hook-driven completion; run `totem --version` and update if background tasks are missing.
...
Background tasks are not complete until Totem injects their terminal result or
...
| Field | Meaning |
|-------|---------|
| task ID | Native Totem background task/session ID |
...
unavailable, treat it as an environment or Totem-version issue rather than an
intentional V1 fallback:

```text
Background orchestration requires Totem background subagents.
Start Totem with:
...
scheduler-centered operating model for Totem's native background subagents.
