...
Switch agent model presets at runtime without restarting Totem using the `/preset` slash command.
...
1. Define named presets in `totempole.jsonc` under the `presets` field
2. Run `/preset <name>` to switch. The plugin calls the Totem SDK's `config.update()` method, which triggers a server-side cache invalidation
...
The following fields are forwarded to the Totem SDK at runtime:
...
| Method | How | Persists? |
|--------|-----|-----------|
| Config file | Set `"preset": "cheap"` in `totempole.jsonc` | Yes, across restarts |
