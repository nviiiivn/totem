...
Bundled skills are installed by the `totempole` installer and safely
...
| [`totempole`](#totempole) | Plugin configuration and self-improvement guidance | `orchestrator` |
...
source repositories into `.slim/clonedeps/repos/` so Totem can inspect library
...
- Totem todo lists for progress tracking
...
`totempole` is an orchestrator-only skill that teaches agents how to
...
and related Totem config files.
...
that Totem may need a restart for config, prompt, agent, skill, MCP, or plugin
changes to take effect.

Typical requests:

```text
Tune my totempole models for lower cost.
...
```text
This should apply on the next Totem run; restart Totem if you need it immediately.
```
...
installs the tarball into a throwaway app, runs Totem with a sanitized
...
smokes such as Totem 1.17.11 malformed message transform regressions.
...
Control which skills each agent can use in `~/.config/totem/totempole.json` (or `.jsonc`):
