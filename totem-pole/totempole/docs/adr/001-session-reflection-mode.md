...
We are adding a `--sessions` mode to the `/reflect` command in totempole. This mode enables cross-session reflection by analyzing past Totem sessions to find repeated patterns, friction, and improvement opportunities.
...
**Decision:** LLM reads `~/.local/share/totem/log/totem.log` and greps for `session.id=ses_[a-f0-9]+` to extract session IDs.

**Rationale:**
- Session IDs are reliably present in the main Totem log
...
**Decision:** Store reflection summaries in `~/.config/totem/totempole/reflections/`.
...
**Structure:**
```
~/.config/totem/totempole/reflections/
  sessions/
...
|--------|------|------|----------|
| `~/.config/totem/totempole/reflections/` | Existing directory, single location | Mixes config and data | **Accepted** |
| `~/.local/share/totempole/reflections/` | XDG-compliant, data separation | New directory, splits OMOS data | Rejected |
