...
async function waitForHealth(url: string, timeoutMs: number) {
  ...
  fail(`Totem server did not become healthy: ${lastError}`);
}
...
function assertNoPluginLoadErrors(logs: string) {
  ...
  fail(
    `Totem logs contain plugin load errors:${relevantLines ? `\n${relevantLines}` : ''}`,
  );
}

function omitTotemEnv(env: NodeJS.ProcessEnv) {
  return Object.fromEntries(
    Object.entries(env).filter(([key]) => !key.startsWith('TOTEM_')),
  );
}

async function verifyHostSmoke(tarballPath: string) {
  const tempRoot = mkdtempSync(path.join(tmpdir(), 'omos-totem-smoke-'));
  ...
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        { name: 'verify-totem-host-smoke', private: true },
        null,
        2,
      ),
    );

    console.log('Installing totem-ai into isolated test root...');
    run('bun', ['add', 'totem-ai@latest'], { cwd: hostDir });

    const totemBin = path.join(hostDir, 'node_modules', '.bin', 'totem');
    if (!existsSync(totemBin)) {
      fail(`Expected totem binary at ${totemBin}`);
    }
    ...
    writeFileSync(
      path.join(pluginDir, 'load-totempole.js'),
      "export { default } from 'totempole';\n",
    );
    ...
    const env = {
      ...
      TOTEM_TEST_HOME: homeDir,
      TOTEM_CONFIG_DIR: configDir,
      TOTEM_CONFIG_CONTENT: config,
      TOTEM_DISABLE_AUTOUPDATE: 'true',
      TOTEM_DISABLE_MODELS_FETCH: 'true',
      TOTEM_DISABLE_DEFAULT_PLUGINS: 'true',
    };

    console.log('Starting totem serve with packaged plugin...');
    const child = spawn(
      totemBin,
      [
        'serve',
    ...
    const exitPromise = new Promise<never>((_, reject) => {
      child.once('exit', (code, signal) => {
        reject(
          new Error(
            `totem serve exited before smoke test completed (code=${code}, signal=${signal})\n${stdout}\n${stderr}`,
          ),
        );
      });
    });
...
async function main() {
  ...
  console.log('Totem host smoke verification passed.');
}
