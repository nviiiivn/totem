...
- `generate-schema.ts`
  ...
  - Writes `totempole.schema.json` with explicit `$schema`, `title`, and plugin description.
...
- `verify-totem-host-smoke.ts`
  - Builds temporary Totem environment (bin from `bun add totem-ai`), mounts the plugin tarball,
    launches `totem serve`, and probes `http://127.0.0.1:<port>/global/health`.
...
- `bun run verify:host-smoke` runs `verify-totem-host-smoke.ts`: pack tarball -> boot isolated host -> wait for health -> verify no plugin-load errors.
