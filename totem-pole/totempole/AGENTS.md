...
**totempole** - A lightweight agent orchestration plugin for Totem, a slimmed-down fork of totempole. Built with TypeScript, Bun, and Biome.
...
| `bun run dev` | Build and run with Totem |
...
# 2. Run from local fork (in ~/.config/totem/totem.jsonc):
# "plugin": ["file:///path/to/totempole"]
...
# 4. Verify no orphans
ps aux | grep "totem attach" | grep -v grep
...
**Orphaned totem attach processes:**
...
Totem has a built-in `/review` command that automatically performs comprehensive code reviews:
...
- This is an Totem plugin - most functionality lives in `src/`
...
Windows: Press WIN+R and paste %USERPROFILE%\.local\share\totem\log
Log files are named with timestamps (e.g., 2025-01-09T123456.log) and the most recent 10 log files are kept.
You can set the log level with the --log-level command-line option to get more detailed debug information. For example, totem --log-level DEBUG.
### Plugin
~/.local/share/totem/log/totempole.<timestamp>.log
...
- `.slim/clonedeps/repos/totem-ai__totem/` - `https://github.com/totem-ai/totem.git` at `main@73ee493265acf15fcd8caab2bc8cd3bd375b63cb`; inspect `packages/plugin` and `packages/sdk/js` for Totem plugin and SDK internals.
- `.slim/clonedeps/repos/totem/` - `https://github.com/anomalyco/totem.git` at `dev@356f6841865d68adf6d0123c37357ad50814497a`; inspect `packages/totem` for latest TypeScript runtime internals and experimental background subagent support.
