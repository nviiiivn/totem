...
import { createTotem, type ToolPart } from "@totem-ai/sdk"
...
console.log("🚀 Starting totem server...")
const totem = await createTotem({
  port: 0,
})
console.log("✅ Totem server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await totem.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new totem session...")
    const { client, server } = totem
    ...
    console.log("✅ Created totem session:", createResult.data.id)
  ...
  console.log("📝 Sending to totem:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Totem response:", JSON.stringify(result, null, 2))
