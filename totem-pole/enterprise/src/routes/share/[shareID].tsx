import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@totem-ai/sdk/v2"
import { SessionTurn } from "@totem-ai/session-ui/session-turn"
import { SessionReview } from "@totem-ai/session-ui/session-review"
import { DataProvider } from "@totem-ai/session-ui/context"
import { FileComponentProvider } from "@totem-ai/ui/context/file"
import { WorkerPoolProvider } from "@totem-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@totem-ai/ui/logo"
import { IconButton } from "@totem-ai/ui/icon-button"
import { ProviderIcon } from "@totem-ai/ui/provider-icon"
import { iife } from "@totem-ai/core/util/iife"
import { Binary } from "@totem-ai/core/util/binary"
import { NamedError } from "@totem-ai/core/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import NotFound from "../[...404]"
import { Tabs } from "@totem-ai/ui/tabs"
import { MessageNav } from "@totem-ai/session-ui/message-nav"
import { FileSSR } from "@totem-ai/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@totem-ai/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/totem-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Totem</Title>
              </Show>
              <Meta name="description" content="totem - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/anomalyco/totem"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
