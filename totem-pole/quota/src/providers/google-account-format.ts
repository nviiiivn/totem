...
function getGoogleAccountLocalPart(email: string): string {
...
export function formatGoogleAccountLabel(
  ...
  _style: GoogleAccountLabelStyle,
): string {
...
export function createGoogleAccountLabelMap(
  ...
  const labels = new Map(
    uniqueEmails.map((email) => [email, formatGoogleAccountLabel(email, style)]),
  );
  const emailsByLabel = new Map<string, string[]>();
  ...
  for (const collidingEmails of emailsByLabel.values()) {
    if (collidingEmails.length < 2) continue;
    ...
    const emailsByPrefix = new Map<string, string[]>();
...
export function formatGoogleAccountErrors(
  ...
  return errors.map((error) => ({
    label:
      (error.email && labels?.get(error.email)) ?? formatGoogleAccountLabel(error.email, style),
    message: error.error,
  }));
}
