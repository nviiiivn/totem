...
Puoi aggiungere strumenti esterni a Totem usando il _Model Context Protocol_, o MCP. Totem supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Totem Config](https://totem.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/totem`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Totem gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Totem:
...
Se il server richiede autenticazione, Totem ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `totem mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Totem memorizzera' i token in modo sicuro in `~/.local/share/totem/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Totem insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Totem come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Totem al tuo account Sentry.
