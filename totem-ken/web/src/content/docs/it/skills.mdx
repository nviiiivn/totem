...
Le skill degli agenti permettono a Totem di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Totem cerca in queste posizioni:

- Config di progetto: `.totem/skills/<name>/SKILL.md`
- Config globale: `~/.config/totem/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Totem risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.totem/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/totem/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.totem/skills/git-release/SKILL.md` cosi':
...
Totem elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `totem.json`:
...
**Per agenti integrati** (in `totem.json`):
