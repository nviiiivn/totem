---
title: Plugin
description: Scrivi plugin per estendere Totem.
---

I plugin ti permettono di estendere Totem agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Totem.
...
- `.totem/plugins/` - plugin a livello progetto
- `~/.config/totem/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/totem/node_modules/`.
...
1. Config globale (`~/.config/totem/totem.json`)
2. Config di progetto (`totem.json`)
3. Directory plugin globale (`~/.config/totem/plugins/`)
4. Directory plugin di progetto (`.totem/plugins/`)
...
Totem esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di totem per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere totem.
...
:::note
Se usi l'app desktop di Totem, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a totem di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a totem:
...
L'helper `tool` crea uno strumento personalizzato che totem può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in totem insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://totem.ai/docs/sdk) per i dettagli.
