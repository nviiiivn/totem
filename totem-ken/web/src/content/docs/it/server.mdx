---
title: Server
description: Interagisci con il server totem via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `totem serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client totem.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `totem.local` |
...
Imposta `TOTEM_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `totem`, oppure imposta `TOTEM_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `totem serve` sia per `totem web`.
...
Quando esegui `totem` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server totem per interagire con totem in modo programmatico.
:::

Questa architettura permette a totem di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `totem serve` per avviare un server standalone. Se la TUI di totem e gia in esecuzione, `totem serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Totem.
...
Il server totem espone le seguenti API.
