...
Tools allow the LLM to perform actions in your codebase. Totem comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Totem.
...
:::note
This tool is only available when `TOTEM_EXPERIMENTAL_LSP_TOOL=true` (or `TOTEM_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Totem provider or when the `TOTEM_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Totem:
