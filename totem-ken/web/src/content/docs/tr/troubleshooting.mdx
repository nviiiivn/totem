...
totem ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\totem\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `totem --log-level DEBUG`.

---

## Depolama

totem, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/totem/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\totem` yapistirin
...
totem Desktop arka planda yerel bir totem sunucusu (`totem-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `totem` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/totem/totem.jsonc` (veya `~/.config/totem/totem.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/totem/totem.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\totem\totem.jsonc` yapistirin
...
totem diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/totem/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\totem\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), totem'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. totem Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/totem`
- **Linux**: `~/.cache/totem` dizinini silin (veya `rm -rf ~/.cache/totem` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\totem` yapistirin

3. totem Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

totem Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`totem.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `TOTEM_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `TOTEM_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta totem Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, totem ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

totem Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda totem icin bildirimler etkinse
...
1. totem Desktop'u kapatin
2. Su dosyalari bulun ve silin (totem Desktop uygulama veri dizininde yer alirlar):

- `totem.settings.dat` (desktop varsayilan sunucu URL'si)
- `totem.global.dat` ve `totem.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
totem ile ilgili bir sorun yasiyorsaniz:
...
3. `totem upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `totem models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\totem`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. totem, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\totem`

2. En guncel provider paketlerini yeniden kurmak icin totem'u yeniden baslatin

Bu, totem'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
totem, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
