...
Totem Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Totem'daki diğer sağlayıcılar gibi çalışır. Totem Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Totem'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Totem Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Totem Go, Totem'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Totem Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Totem Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Totem Go'ya abone olabilir.
:::
...
Totem Go aşağıdaki limitleri içerir:
...
Totem yapılandırmanızdaki [model id](/docs/config/#models) formatı `totem-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `totem-go/kimi-k2.7-code` kullanmalısınız.
...
Totem Go'yu şu amaçlarla oluşturduk:
...
4. Totem ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
