---
title: Eklentiler
description: Totem'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Totem'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Totem'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.totem/plugins/` - Proje düzeyinde eklentiler
- `~/.config/totem/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/totem/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/totem/totem.json`)
2. Project config (`totem.json`)
3. Global eklenti dizini (`~/.config/totem/plugins/`)
4. Proje eklenti dizini (`.totem/plugins/`)
...
Totem bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Totem SDK istemcisi.
...
Totem'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Totem masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Totem'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Totem'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Totem'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Totem'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://totem.ai/docs/sdk) bakın.
