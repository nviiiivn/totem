...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. totem yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
totem içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `TOTEM_EXPERIMENTAL_LSP_TOOL=true` (veya `TOTEM_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca totem provider kullanılırken veya `TOTEM_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

totem başlatırken etkinleştirmek için:
