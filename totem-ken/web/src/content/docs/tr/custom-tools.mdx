---
title: Özel Araçlar
description: LLM'un totem'da çağırabileceği araçları birleştirebilir.
---

Özel araçlar, LLM'un konuşmalar sırasında arayabileceği, oluşturduğunuz işlevlerdir. totem'un [built-in tools](/docs/tools) ile birlikte `read`, `write` ve `bash` gibi çalışırlar.
...
- Yerel olarak bunları projenizin `.totem/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/totem/tools/` içine yerleştirerek.
