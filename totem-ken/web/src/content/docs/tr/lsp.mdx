...
description: Totem, LSP sunucularınızla bütünleşir.
---

Totem, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Totem, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`TOTEM_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve totem bir dosyayı açtığında:
...
LSP sunucularını, totem yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
