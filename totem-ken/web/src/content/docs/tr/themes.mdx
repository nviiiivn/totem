...
totem ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak totem kendi `totem` temasını kullanır.
...
totem birden fazla yerleşik temayla gelir.
...
- totem'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
totem, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/totem/themes/*.json` veya `$XDG_CONFIG_HOME/totem/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.totem/themes/*.json`
4. **Geçerli çalışma dizini** - `./.totem/themes/*.json`
