...
_Model Bağlam Protokolü_ veya MCP kullanarak totem'a harici araçlar ekleyebilirsiniz. totem hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [totem Config](https://totem.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/totem` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
totem, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde totem şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa totem, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `totem mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından totem, belirteçleri `~/.local/share/totem/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra totem'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi totem yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve totem'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
