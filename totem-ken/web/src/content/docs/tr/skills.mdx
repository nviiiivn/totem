...
Ajan becerileri, totem'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
totem su konumlari tarar:

- Proje konfigurasyonu: `.totem/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/totem/skills/<name>/SKILL.md`
...
Proje yerel yollarinda totem, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.totem/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/totem/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.totem/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
totem, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`totem.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`totem.json` icinde):
