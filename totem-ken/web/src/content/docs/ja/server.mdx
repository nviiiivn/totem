---
title: サーバー
description: HTTP 経由で Totem サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`totem serve` コマンドは、Totem クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `totem.local` |
...
HTTP 基本認証でサーバーを保護するには、`TOTEM_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `totem` になるか、`TOTEM_SERVER_USERNAME` を設定してオーバーライドします。これは、`totem serve` と `totem web` の両方に当てはまります。
...
`totem` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Totem サーバーを使用して、プログラムで Totem と対話します。
:::

`totem serve` を実行してスタンドアロンサーバーを起動できます。すでに Totem TUI を実行している場合でも、`totem serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Totem [IDE](/docs/ide) プラグインによって使用されます。
