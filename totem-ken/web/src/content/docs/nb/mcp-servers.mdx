...
Du kan legge til eksterne verktøy i Totem ved å bruke _Model Context Protocol_ eller MCP. Totem støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Totem Config](https://totem.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="totem.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/totem`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Totem håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Totem:
...
Hvis serveren krever autentisering, vil Totem be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `totem mcp auth <server-name>`.
...
```bash
totem mcp auth my-oauth-server
```
...
```bash
totem mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Totem lagre tokens sikkert i `~/.local/share/totem/mcp-auth.json`.
...
totem mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Totem, sammen med innebygde verktøy. Så du kan administrere dem gjennom Totem-konfigurasjonen som alle andre verktøy.
...
```json title="totem.json" {11, 14-18}
{
  "$schema": "https://totem.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
totem mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Totem til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Totem-samtale oppført som en separat økt.
