...
description: Skriv dine egne programtillegg for å utvide Totem.
---

Plugins lar deg utvide Totem ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Totem.
...
- `.totem/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/totem/node_modules/`.
...
1. Global konfigurasjon (`~/.config/totem/totem.json`)
2. Prosjektkonfigurasjon (`totem.json`)
3. Global plugin-katalog (`~/.config/totem/plugins/`)
4. Prosjektpluginkatalog (`.totem/plugins/`)
...
Totem kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Totem SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Totem.
...
:::note
Hvis du bruker Totem-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Totem fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Totem:
...
`tool`-hjelperen lager et tilpasset verktøy som Totem kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Totem sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://totem.ai/docs/sdk) for detaljer.
