---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Totem.
---

import config from "../../../../config.mjs"
export const console = config.console

Totem bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Totem-konfigurasjonen.
...
i `~/.local/share/totem/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Totem
konfig.
...
Totem Zen er en liste over modeller levert av Totem-teamet som har vært
testet og verifisert for å fungere godt med Totem. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Totem Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Totem Zen` og gå til [totem.ai/auth](https://totem.ai/zen).
...
Det fungerer som alle andre leverandører i Totem og er helt valgfritt å bruke.

---

## Totem Go

Totem Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Totem-teamet som har vært
testet og verifisert for å fungere godt med Totem.

1. Kjør kommandoen `/connect` i TUI, velg `Totem Go`, og gå til [totem.ai/auth](https://totem.ai/zen).
...
Slik bruker du Amazon Bedrock med Totem:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Totem:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `totem.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i totem støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere totem til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Totem skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Totem-konfigurasjonen.
...
for å lagre token i Totem auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Totem bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Totem
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`totem.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [totem-gitlab-auth](https://www.npmjs.com/package/totem-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `totem.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Totem:
...
Slik bruker du Google Vertex AI med Totem:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Totem, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [totem-helicone-session](https://github.com/H2Shami/totem-helicone-session) plugin for automatisk å logge hver Totem-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Totem-samtale oppført som en separat økt.
...
Du kan konfigurere Totem for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Totem for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Totem for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Totem. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/totem) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Totem:
...
3. Kopier API-nøkkelen for bruk i Totem.
...
6. **Viktig**: Før du bruker skymodeller i Totem, må du hente modellinformasjonen lokalt:
...
Totem Zen er en liste over testede og verifiserte modeller levert av Totem-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Totem Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Totem Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Totem API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Totem-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Totem-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Totem:
...
Du kan også tilpasse modeller gjennom Totem-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Totem. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `totem.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Totem forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `totem auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Totem-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Totem-konfigurasjonen.
