...
Her er alle de innebygde verktøyene som er tilgængelige i Totem.
...
:::note
Dette verktøyet er bare tilgængelig når `TOTEM_EXPERIMENTAL_LSP_TOOL=true` (eller `TOTEM_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Totem-leverandøren eller når miljøvariabelen `TOTEM_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
