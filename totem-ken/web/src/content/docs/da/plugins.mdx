---
title: Plugins
description: Skriv dine egne plugins for at udvide Totem.
---

Plugins giver dig mulighed for at udvide Totem ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Totems standardadfærd.
...
- `.totem/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/totem/node_modules/`.
...
1. Global konfiguration (`~/.config/totem/totem.json`)
2. Projektkonfiguration (`totem.json`)
3. Global plugin-mappe (`~/.config/totem/plugins/`)
4. Projekt plugin bibliotek (`.totem/plugins/`)
...
Totem kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En totem SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide totem.
...
:::note
Hvis du bruger Totem desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå totem i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til totem:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som totem kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for totem sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://totem.ai/docs/sdk) for detaljer.
