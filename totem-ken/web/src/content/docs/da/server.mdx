---
title: Server
description: Interager med totem-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `totem serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en totem-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `totem.local` |
...
Indstil `TOTEM_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `totem`, eller indstil `TOTEM_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `totem serve` og `totem web`.
...
Når du kører `totem` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug totem-serveren til at interagere med totem programmatisk.
:::

Denne arkitektur lader totem understøtte flere klienter og lader dig interagere med totem programmatisk.

Du kan køre `totem serve` for at starte en selvstændig server. Hvis du har
totem TUI kørende, vil `totem serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Totem [IDE](/docs/ide) plugins.
...
Totem-serveren viser følgende API'er.
