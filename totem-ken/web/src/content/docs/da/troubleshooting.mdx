...
For at fejlfinde problemer med Totem, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\totem\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `totem --log-level DEBUG`.

---

## Lagring

totem gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/totem/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\totem`
...
Totem Desktop kører en lokal Totem-server (`totem-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Totem`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/totem/totem.jsonc` (eller `~/.config/totem/totem.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/totem/totem.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\totem\totem.jsonc`
...
Totem kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/totem/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\totem\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Totem kan genopbygge den.

1. Afslut Totem Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/totem`
- **Linux**: slet `~/.cache/totem` (eller kør `rm -rf ~/.cache/totem`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\totem`

3. Start Totem Desktop på ny.

---

### Løs problemer med serverforbindelse

Totem Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `totem.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `TOTEM_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `TOTEM_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Totem Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Totem.

---

### Meddelelser vises ikke

Totem Desktop viser kun systemvarsler når:

- varsler er aktiveret for Totem i dine OS-indstillinger, og
...
1. Afslut Totem Desktop.
2. Find og slet disse filer (de findes i Totem Desktop-appens datamappe):

- `totem.settings.dat` (skrivebordsstandardserver URL)
- `totem.global.dat` og `totem.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Totem:
...
For at finde ud af hvilke modeller du har adgang til, kør `totem models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\totem`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. totem installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\totem`

2. Start totem på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge totem til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
totem vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
