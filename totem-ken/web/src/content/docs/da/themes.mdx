...
Med Totem kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Totem vårt eget `totem`-tema.
...
Totem kommer med flere innebygde temaer.
...
- Vil at Totem skal matche terminalens utseende
...
Totem støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/totem/themes/*.json` eller `$XDG_CONFIG_HOME/totem/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.totem/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.totem/themes/*.json`
