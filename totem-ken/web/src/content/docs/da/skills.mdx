...
Agentferdigheter lar Totem oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Totem søker etter disse stedene:

- Prosjektkonfigurasjon: `.totem/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/totem/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Totem opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.totem/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/totem/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.totem/skills/git-release/SKILL.md` slik:
...
Totem viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `totem.json`:
...
**For innebygde agenter** (i `totem.json`):
