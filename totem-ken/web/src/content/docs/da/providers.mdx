---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Totem.
---

import config from "../../../../config.mjs"
export const console = config.console

Totem bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Totem-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/totem/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Totem-konfigurationen.
...
Totem Zen er en liste over modeller leveret af Totem-teamet, som er blevet testet og verificeret til at fungere godt med Totem. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Totem Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg totem og gå til [totem.ai/auth](https://totem.ai/auth).
...
Det fungerer som alle andre udbydere i Totem og er helt valgfrit at bruge.

---

## Totem Go

Totem Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Totem-teamet, som er testet og verificeret til at fungere godt med Totem.

1. Kør kommandoen `/connect` i TUI, vælg `Totem Go`, og gå til [totem.ai/auth](https://totem.ai/zen).
...
Sådan bruger du Amazon Bedrock med Totem:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører totem:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `totem.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Totem understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere totem til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at totem skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem totem-konfigurationen.
...
gemme tokenet i totem auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Totem bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Totem
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`totem.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [totem-gitlab-auth](https://www.npmjs.com/package/totem-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `totem.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Totem:
...
Sådan bruger du Google Vertex AI med Totem:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem totem, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [totem-helicone-session](https://github.com/H2Shami/totem-helicone-session) plugin for automatisk at logge hver Totem-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Totem-samtale opført som en separat session.
...
Du kan konfigurere totem til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere totem til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere totem til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Totem. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/totem) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Totem:
...
3. Kopier API-nøglen til brug i Totem.
...
6. **Vigtigt**: Før du bruger skymodeller i Totem, skal du hente modelinformationen lokalt:
...
Totem Zen er en liste over testede og verificerede modeller leveret af Totem-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Totem Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Totem Zen**.

   ```txt
   /connect
   ```

3. Indtast Totem API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem totem-konfigurationen.
...
5. Du kan også tilpasse dem gennem totem-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Totem:
...
Du kan også tilpasse modeller gennem totem-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med totem. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `totem.json`-filen i projektkataloget:
...
`limit`-felterne lader Totem forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `totem auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek totem-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i totem-konfigurationen.
