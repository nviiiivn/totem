...
Com o totem, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o totem usa nosso próprio tema `totem`.
...
O totem vem com vários temas embutidos.
...
- Querem que o totem corresponda à aparência do seu terminal
...
O totem suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/totem/themes/*.json` ou `$XDG_CONFIG_HOME/totem/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.totem/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.totem/themes/*.json`
