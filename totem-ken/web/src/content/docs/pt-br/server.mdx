---
title: Servidor
description: Interaja com o servidor totem via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `totem serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente totem pode usar.
...
```bash
totem serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `totem.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
totem serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `TOTEM_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `totem`, ou defina `TOTEM_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `totem serve` quanto ao `totem web`.

```bash
TOTEM_SERVER_PASSWORD=your-password totem serve
```
...
Quando você executa `totem`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor totem para interagir com o totem programaticamente.
:::

Essa arquitetura permite que o totem suporte múltiplos clientes e permite que você interaja com o totem programaticamente.

Você pode executar `totem serve` para iniciar um servidor autônomo. Se você tiver o TUI do totem em execução, `totem serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do totem [IDE](/docs/ide).
...
O servidor totem expõe as seguintes APIs.
