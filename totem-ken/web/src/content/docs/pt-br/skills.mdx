...
As habilidades do agente permitem que o totem descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O totem pesquisa nesses locais:

- Configuração do projeto: `.totem/skills/<name>/SKILL.md`
- Configuração global: `~/.config/totem/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o totem sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.totem/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/totem/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.totem/skills/git-release/SKILL.md` assim:
...
O totem lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `totem.json`:
...
**Para agentes embutidos** (em `totem.json`):
