...
As ferramentas permitem que o LLM execute ações em sua base de código. O totem vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no totem.
...
:::note
Esta ferramenta está disponível apenas quando `TOTEM_EXPERIMENTAL_LSP_TOOL=true` (ou `TOTEM_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor totem ou quando a variável de ambiente `TOTEM_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o totem:
