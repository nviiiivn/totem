---
title: Plugins
description: Escreva seus próprios plugins para estender o totem.
---

Plugins permitem que você estenda o totem conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do totem.
...
- `.totem/plugins/` - Plugins em nível de projeto
- `~/.config/totem/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/totem/node_modules/`.
...
1. Configuração global (`~/.config/totem/totem.json`)
2. Configuração do projeto (`totem.json`)
3. Diretório de plugins global (`~/.config/totem/plugins/`)
4. Diretório de plugins do projeto (`.totem/plugins/`)
...
O totem executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do totem para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o totem.
...
:::note
Se você estiver usando o aplicativo desktop totem, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o totem de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao totem:
...
O helper `tool` cria uma ferramenta personalizada que o totem pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o totem junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://totem.ai/docs/sdk) para detalhes.
