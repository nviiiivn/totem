...
Você pode adicionar ferramentas externas ao totem usando o _Model Context Protocol_, ou MCP. O totem suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do totem](https://totem.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/totem`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O totem lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o totem irá:
...
Se o servidor requer autenticação, o totem solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `totem mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o totem armazenará os tokens de forma segura em `~/.local/share/totem/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no totem, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do totem como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o totem à sua conta do Sentry.
