---
title: Servidor
description: Interactuar con el servidor totem a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `totem serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente totem puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `totem.local` |
...
Configure `TOTEM_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `totem`, o configure `TOTEM_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `totem serve` como a `totem web`.
...
Cuando ejecuta `totem`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor totem para interactuar con totem mediante programación.
:::

Esta arquitectura permite que totem admita múltiples clientes y le permite interactuar con totem mediante programación.

Puede ejecutar `totem serve` para iniciar un servidor independiente. Si tienes el
totem TUI ejecutándose, `totem serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Totem [IDE](/docs/ide).
...
El servidor totem expone las siguientes API.
