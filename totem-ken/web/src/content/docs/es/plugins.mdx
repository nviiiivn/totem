---
title: Complementos
description: Escriba sus propios complementos para extender Totem.
---

Los complementos le permiten extender Totem al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Totem.
...
- `.totem/plugins/` - Complementos a nivel de proyecto
- `~/.config/totem/plugins/` - Complementos globales
...
```json title="totem.json"
{
  ...
  "plugin": ["totem-helicone-session", "totem-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/totem/node_modules/`.
...
1. Configuración global (`~/.config/totem/totem.json`)
2. Configuración del proyecto (`totem.json`)
3. Directorio global de complementos (`~/.config/totem/plugins/`)
4. Directorio de complementos del proyecto (`.totem/plugins/`)
...
Totem ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".totem/plugins/my-plugin.ts"
...
```js title=".totem/plugins/example.js"
...
- `client`: Un cliente SDK totem para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar totem.
...
```js title=".totem/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "totem"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Totem, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que totem lea archivos `.env`:

```javascript title=".totem/plugins/env-protection.js"
...
```javascript title=".totem/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a totem:

```ts title=".totem/plugins/custom-tools.ts"
import { type Plugin, tool } from "@totem-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que totem puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para totem junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://totem.ai/docs/sdk) para obtener más detalles.
...
```ts title=".totem/plugins/compaction.ts"
import type { Plugin } from "@totem-ai/plugin"
...
```ts title=".totem/plugins/custom-compaction.ts"
import type { Plugin } from "@totem-ai/plugin"
