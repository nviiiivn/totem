...
Las herramientas permiten que LLM realice acciones en su código base. Totem viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Totem.
...
:::note
Esta herramienta solo está disponible cuando `TOTEM_EXPERIMENTAL_LSP_TOOL=true` (o `TOTEM_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Totem o cuando la variable de entorno `TOTEM_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Totem:

```bash
TOTEM_ENABLE_EXA=1 totem
```
