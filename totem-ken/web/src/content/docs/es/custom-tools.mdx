...
description: Cree herramientas que LLM pueda llamar en totem.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de totem como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.totem/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/totem/tools/`.
...
```ts title=".totem/tools/database.ts" {1}
...
```ts title=".totem/tools/project.ts" {8}
import { tool } from "@totem-ai/plugin"
...
```python title=".totem/tools/add.py"
import sys
...
```ts title=".totem/tools/python-add.ts" {10}
import { tool } from "@totem-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".totem/tools/add.py")
