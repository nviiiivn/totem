...
Con Totem puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Totem usa nuestro propio tema `totem`.
...
Totem viene con varios temas integrados.
...
- Quiere que Totem coincida con la apariencia de su terminal
...
Totem admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/totem/themes/*.json` o `$XDG_CONFIG_HOME/totem/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.totem/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.totem/themes/*.json`
