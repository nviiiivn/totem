...
Las habilidades del agente permiten a Totem descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Totem busca estas ubicaciones:

- Configuración del proyecto: `.totem/skills/<name>/SKILL.md`
- Configuración global: `~/.config/totem/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Totem sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.totem/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/totem/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.totem/skills/git-release/SKILL.md` así:
...
Totem enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `totem.json`:
...
**Para agentes integrados** (en `totem.json`):
