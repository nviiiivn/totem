---
title: Serveur
description: Interagissez avec le serveur totem via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `totem serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client totem peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `totem.local` |
...
Définissez `TOTEM_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `totem`, ou définissez `TOTEM_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `totem serve` et à `totem web`.
...
Lorsque vous exécutez `totem`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur totem pour interagir avec totem par programmation.
:::

Cette architecture permet à totem de prendre en charge plusieurs clients et vous permet d'interagir avec totem par programme.

Vous pouvez exécuter `totem serve` pour démarrer un serveur autonome. Si vous avez le
totem TUI en cours d'exécution, `totem serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Totem [IDE](/docs/ide).
...
Le serveur totem expose les API suivantes.
