...
Avec Totem, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Totem utilise notre propre thème `totem`.
...
Totem est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Totem corresponde à l'apparence de leur terminal
...
Totem prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/totem/themes/*.json` ou `$XDG_CONFIG_HOME/totem/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.totem/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.totem/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/totem/themes
vim ~/.config/totem/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .totem/themes
vim .totem/themes/my-theme.json
```
...
{
  "$schema": "https://totem.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
