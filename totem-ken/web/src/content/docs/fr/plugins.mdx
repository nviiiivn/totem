---
title: Plugins
description: Écrivez vos propres plugins pour étendre Totem.
---

Les plugins vous permettent d'étendre Totem en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Totem.
...
- `.totem/plugins/` - Plugins au niveau du projet
- `~/.config/totem/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/totem/node_modules/`.
...
1. Configuration globale (`~/.config/totem/totem.json`)
2. Configuration du projet (`totem.json`)
3. Répertoire global des plugins (`~/.config/totem/plugins/`)
4. Répertoire des plugins du projet (`.totem/plugins/`)
...
Totem exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK totem pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre totem.
...
:::note
Si vous utilisez l'application de bureau Totem, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez totem de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à totem :
...
L'assistant `tool` crée un outil personnalisé que totem peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour totem aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://totem.ai/docs/sdk) pour plus de détails.
