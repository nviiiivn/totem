---
title: SDK
description: Client JS de type sécurisé pour le serveur totem.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK totem JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler totem par programme.
...
Créez une instance de totem :

```javascript
import { createTotem } from "@totem-ai/sdk"

const { client } = await createTotem()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `totem.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${totem.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de totem, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@totem-ai/sdk"
```
