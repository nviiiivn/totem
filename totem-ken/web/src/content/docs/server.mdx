---
title: Server
description: Interact with totem server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `totem serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an totem client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `totem.local` |
...
Set `TOTEM_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `totem`, or set `TOTEM_SERVER_USERNAME` to override it. This applies to both `totem serve` and `totem web`.
...
When you run `totem` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the totem server to interact with totem programmatically.
:::

This architecture lets totem support multiple clients and allows you to interact with totem programmatically.

You can run `totem serve` to start a standalone server. If you have the
totem TUI running, `totem serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Totem [IDE](/docs/ide) plugins.
...
The totem server exposes the following APIs.
