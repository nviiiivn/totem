---
title: Policies
description: Control which configured resources Totem may use.
---

Policies control whether Totem may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `totem.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether Totem may use a resource such as an LLM provider.
...
Totem currently supports one policy action:
