---
title: المزوّدون
description: استخدام أي موفّر LLM في Totem.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Totem ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Totem.
...
في `~/.local/share/totem/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Totem.
...
Totem Zen هي قائمة نماذج يوفّرها فريق Totem وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Totem. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Totem Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر totem، ثم انتقل إلى [totem.ai/auth](https://totem.ai/auth).
...
يعمل مثل أي مزوّد آخر في Totem واستخدامه اختياري بالكامل.

---

## Totem Go

Totem Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Totem، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Totem.

1. شغّل الأمر `/connect` في TUI، واختر `Totem Go`، ثم انتقل إلى [totem.ai/auth](https://totem.ai/zen).
...
لاستخدام Amazon Bedrock مع Totem:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل totem:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `totem.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Totem غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين totem لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل totem بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات totem.
...
تخزين الرمز في مساحة تخزين مصادقة totem.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Totem نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Totem
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`totem.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [totem-gitlab-auth](https://www.npmjs.com/package/totem-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع totem:
...
لاستخدام Google Vertex AI مع Totem:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر totem، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [totem-helicone-session](https://github.com/H2Shami/totem-helicone-session) لتسجيل كل محادثة Totem تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Totem مدرجة كجلسة مستقلة.
...
يمكنك ضبط totem لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط totem لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط totem لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Totem. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/totem) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Totem:
...
3. انسخ مفتاح API لاستخدامه في Totem.
...
6. **مهم**: قبل استخدام نماذج السحابة في Totem، يجب سحب معلومات النموذج محليا:
...
Totem Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Totem. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Totem Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Totem Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Totem.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات totem.
...
5. يمكنك أيضا تخصيصها عبر إعدادات totem. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Totem:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات totem. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع totem. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `totem.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Totem بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `totem auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات totem وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات totem.
