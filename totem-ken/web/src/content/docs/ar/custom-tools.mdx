...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في totem.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في totem مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.totem/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/totem/tools/`.
