---
title: الخادم
description: تفاعل مع خادم totem عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `totem serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل totem استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `totem.local` |
...
عيّن `TOTEM_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `totem`، أو عيّن `TOTEM_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `totem serve` و `totem web`.
...
عند تشغيل `totem` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم totem للتفاعل مع totem برمجيا.
:::

تتيح هذه البنية لـ totem دعم عدة عملاء وتمكّنك من التفاعل مع totem برمجيا.

يمكنك تشغيل `totem serve` لبدء خادم مستقل. إذا كانت واجهة totem في terminal (TUI)
قيد التشغيل، فسيبدأ `totem serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Totem لـ [IDE](/docs/ide).
...
يعرض خادم totem واجهات API التالية.
