...
تتيح مهارات الوكيل لـ Totem اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Totem في هذه المواقع:

- إعدادات المشروع: `.totem/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/totem/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Totem صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.totem/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/totem/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.totem/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Totem المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `totem.json`:
...
**للوكلاء المدمجين** (في `totem.json`):
