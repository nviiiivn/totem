...
مع Totem يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Totem سمتنا `totem`.
...
- يريدون أن يطابق Totem مظهر terminal لديهم
...
يدعم Totem نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/totem/themes/*.json` أو `$XDG_CONFIG_HOME/totem/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.totem/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.totem/themes/*.json`
