...
أنشئ الملف `.totem/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Totem أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Totem:
...
- على مستوى النظام: `~/.config/totem/commands/`
- لكل مشروع: `.totem/commands/`
...
يتضمن totem عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
