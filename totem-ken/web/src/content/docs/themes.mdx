...
With Totem you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Totem uses our own `totem` theme.
...
Totem comes with several built-in themes.
...
- Want Totem to match their terminal's appearance
...
Totem supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/totem/themes/*.json` or `$XDG_CONFIG_HOME/totem/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.totem/themes/*.json`
4. **Current working directory** - Defined in `./.totem/themes/*.json`
