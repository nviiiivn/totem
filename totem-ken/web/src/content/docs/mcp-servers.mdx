...
You can add external tools to Totem using the _Model Context Protocol_, or MCP. Totem supports both local and remote servers.
...
You can define MCP servers in your [Totem Config](https://totem.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/totem` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Totem automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Totem will:
...
If the server requires authentication, Totem will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `totem mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Totem will store the tokens securely in `~/.local/share/totem/mcp-auth.json`.
...
Your MCPs are available as tools in Totem, alongside built-in tools. So you can manage them through the Totem config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Totem to your Sentry account.
