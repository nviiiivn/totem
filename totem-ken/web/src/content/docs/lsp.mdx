---
title: LSP Servers
description: Totem integrates with your LSP servers.
---

Totem can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Totem comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `TOTEM_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and totem opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your totem config.
