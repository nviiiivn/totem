...
Wenn Totem Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\totem\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `totem --log-level DEBUG`.

---

## Speicher

totem speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/totem/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\totem`
...
Totem Desktop startet im Hintergrund einen lokalen Totem-Server (`totem-cli`-Sidecar).
...
- Nur macOS: `Totem`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/totem/totem.jsonc` (oder `~/.config/totem/totem.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/totem/totem.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\totem\totem.jsonc`
...
Totem kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/totem/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\totem\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Totem ihn neu aufbauen kann.

1. Beende Totem Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/totem`
- **Linux**: loesche `~/.cache/totem` (oder fuehre aus: `rm -rf ~/.cache/totem`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\totem`

3. Starte Totem Desktop neu.

---

### Server-Verbindungsprobleme beheben

Totem Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `totem.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `TOTEM_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `TOTEM_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Totem Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Totem-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Totem Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Totem in den OS-Einstellungen aktiviert sind, und
...
1. Beende Totem Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Totem Desktop):

- `totem.settings.dat` (desktop default server URL)
- `totem.global.dat` und `totem.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Totem hast:
...
3. Stelle sicher, dass du die neueste Version hast: `totem upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `totem models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\totem`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. totem installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\totem`

2. Starte totem neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt totem, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
totem erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
