...
Agent Skills erlauben Totem, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Totem sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Totem vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.totem/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/totem/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.totem/skills/git-release/SKILL.md` zum Beispiel so:
...
Totem listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `totem.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `totem.json`):
