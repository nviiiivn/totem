---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Totem zu erweitern.
---

Mit Plugins können Sie Totem erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Totem zu ändern.
...
- `.totem/plugins/` – Plugins auf Projektebene
- `~/.config/totem/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/totem/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/totem/totem.json`)
2. Projektkonfiguration (`totem.json`)
3. Globales Plugin-Verzeichnis (`~/.config/totem/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.totem/plugins/`)
...
Totem führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Totem-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Totem erweitern können.
...
:::note
Wenn Sie die Totem-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Totem `.env`-Dateien liest:
...
Plugins können Totem auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Totem aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Totem zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://totem.ai/docs/sdk).
