...
Totem Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Totem. Du abonnierst Totem Go und
...
Totem Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Totem Go funktioniert wie jeder andere Provider in Totem.

1. Du meldest dich bei **<a href={console}>Totem Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Totem Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Totem Go abonnieren.
:::
...
Totem Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Totem Config verwendet das Format `totem-go/<model-id>`. Für Kimi K2.7 Code würdest du beispielsweise `totem-go/kimi-k2.7-code` in deiner Config verwenden.
...
Wir haben Totem Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Totem zu nutzen.
