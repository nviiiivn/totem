---
title: Server
description: Nutze den totem-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `totem serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein totem-Client nutzen kann.
...
Setze `TOTEM_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `totem`, kann aber mit `TOTEM_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `totem serve` und `totem web`.
...
Wenn du `totem` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den totem-Server, um totem programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `totem serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `totem serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Totem-[IDE](/docs/ide)-Plugins.
...
Der totem-Server stellt folgende APIs bereit.
