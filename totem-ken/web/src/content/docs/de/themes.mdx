...
Mit Totem kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Totem das `totem`-Theme.
...
Totem liefert mehrere eingebaute Themes mit.
...
- Totem optisch ans Terminal anpassen wollen
...
Totem unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/totem/themes/*.json` oder `$XDG_CONFIG_HOME/totem/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.totem/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.totem/themes/*.json`
