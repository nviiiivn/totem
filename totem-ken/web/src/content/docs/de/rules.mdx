---
title: Regeln
description: Lege eigene Anweisungen fuer totem fest.
---

Du kannst totem ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in totem den Befehl `/init` aus.
...
So kann totem sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht totem sie zu erweitern.
...
totem kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/totem/AGENTS.md` speichern.
Diese gelten in allen totem-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Totem dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/totem/AGENTS.md` existiert)
...
Beim Start sucht totem in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/totem/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `totem.json` oder in `~/.config/totem/totem.json` angeben.
...
totem parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit totem.json

Empfohlen ist das `instructions`-Feld in der `totem.json`:
...
Du kannst totem explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `totem.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
