...
Umiejętności agenta pozwalają totem odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
totem przeszukuje te lokalizacje:

- Project config: `.totem/skills/<name>/SKILL.md`
- Global config: `~/.config/totem/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu totem przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.totem/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/totem/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.totem/skills/git-release/SKILL.md` w ten sposób:
...
totem wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `totem.json`:
...
**Dla agentów wbudowanych** (w `totem.json`):
