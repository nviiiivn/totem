...
Możesz dodać dodatkowe informacje do totem za pomocą _Model Context Protocol_, w skrócie MCP. totem obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [totem Config](https://totem.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/totem`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
totem automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, totem:
...
Jeśli serwer wymaga uwierzytelnienia, totem poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `totem mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji totem bezpieczne przechowa tokeny w `~/.local/share/totem/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w totem, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie totem, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć totem z kontem Sentry.
