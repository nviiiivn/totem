---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w totem.
---

import config from "../../../../config.mjs"
export const console = config.console

totem używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji totem.
...
w `~/.local/share/totem/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim totem
config.
...
Totem Zen to lista modeli dostarczonych przez zespół totem, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z totem. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Totem Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Totem Zen` i przejdź do [totem.ai/auth](https://totem.ai/zen).
...
Działa jak każdy inny dostawca w totem i jest całkowicie opcjonalny w użyciu.

---

## Totem Go

Totem Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół totem, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z totem.

1. Uruchom polecenie `/connect` w TUI, wybierz `Totem Go` i przejdź do [totem.ai/auth](https://totem.ai/zen).
...
Aby używać Amazon Bedrock z totem:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania totem:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `totem.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w totem nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować totem do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod totem działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji totem.
...
totem używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować totem
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`totem.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [totem-gitlab-auth](https://www.npmjs.com/package/totem-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem totem:
...
Aby używać Google Vertex AI z totem:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania totem.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą totem, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [totem-helicone-session](https://github.com/H2Shami/totem-helicone-session), aby automatycznie rejestrować każdą konwersację totem jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja totem będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować totem tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować totem tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować totem tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem totem. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/totem).
:::
...
Aby korzystać z Ollama Cloud z totem:
...
3. Skopiuj klucz API do użycia w totem.
...
6. **Ważne**: Przed użyciem modeli chmurowych w totem musisz pobrać informacje o modelu lokalnie:
...
Totem Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół totem. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Totem Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Totem Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API totem.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji totem.
...
5. Możesz je także dostosować za pomocą konfiguracji totem. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z totem:
...
Możesz także dostosować modele za pomocą konfiguracji totem. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z totem. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `totem.json` w katalogu projektu:
...
Pola `limit` pozwalają totem zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `totem auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację totem i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji totem.
