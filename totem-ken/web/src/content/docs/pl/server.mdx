---
title: Serwer
description: Interakcja z serwerem totem za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `totem serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient totem.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `totem.local` |
...
Ustaw `TOTEM_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `totem` lub ustaw `TOTEM_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `totem serve`, jak i `totem web`.
...
Kiedy uruchomisz `totem`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera totem do programowej interakcji z totem.
:::

Ta architektura umożliwia obsługę wielu klientów przez totem i programową interakcję z kodem otwartym.

Możesz uruchomić `totem serve`, aby uruchomić samodzielny serwer. Jeśli masz
totem TUI działa, `totem serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki totem [IDE](/docs/ide).
...
Serwer totem udostępnia następujące interfejsy API.
