...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Totem posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Totem.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `TOTEM_EXPERIMENTAL_LSP_TOOL=true` (lub `TOTEM_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Totem lub gdy zmienna środowiskowa `TOTEM_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Totem:
