...
Dzięki totem możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie totem używa naszego własnego motywu `totem`.
...
totem ma kilka wbudowanych motywów.
...
- Chcą, aby Totem pasował do wyglądu ich terminala
...
totem obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/totem/themes/*.json` lub `$XDG_CONFIG_HOME/totem/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.totem/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.totem/themes/*.json`
