---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć totem.
---

Wtyczki umożliwiające wyłączenie totem poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania totem.
...
- `.totem/plugins/` - Wtyczki na poziomie projektu
- `~/.config/totem/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/totem/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/totem/totem.json`)
2. Konfiguracja projektu (`totem.json`)
3. Globalny katalog wtyczek (`~/.config/totem/plugins/`)
4. Katalog wtyczek projektu (`.totem/plugins/`)
...
totem uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z totem do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do totem.
...
:::note
Jeśli korzystasz z aplikacji komputerowej totem, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj totem czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do totem:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod totem. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://totem.ai/docs/sdk).
