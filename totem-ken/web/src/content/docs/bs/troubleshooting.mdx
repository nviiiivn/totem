...
Da biste otklonili probleme s Totem, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\totem\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `totem --log-level DEBUG`.

---

## Pohrana

Totem pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/totem/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\totem`
...
Totem Desktop pokreće lokalni Totem server (`totem-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Totem` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/totem/totem.jsonc` (ili `~/.config/totem/totem.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/totem/totem.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\totem\totem.jsonc`
...
Totem također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/totem/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\totem\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Totem mogao ponovo izgraditi.

1. Potpuno zatvorite Totem Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/totem`
- **Linux**: obrišite `~/.cache/totem` (ili pokrenite `rm -rf ~/.cache/totem`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\totem`

3. Ponovo pokrenite Totem Desktop.

---

### Rješavanje problema sa vezom na serveru

Totem Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `totem.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `TOTEM_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `TOTEM_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Totem Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Totem karakteristikama.

---

### Obavještenja se ne prikazuju

Totem Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Totem u postavkama vašeg OS-a, i
...
1. Zatvorite Totem Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Totem Desktop aplikacije):

- `totem.settings.dat` (URL zadanog servera za desktop)
- `totem.global.dat` i `totem.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `totem upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `totem models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\totem`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Totem dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\totem`

2. Ponovo pokrenite Totem da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Totem da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Totem će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
