...
Agent skills omogucavaju Totem da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Totem pretrazuje ove lokacije:

- Konfiguracija projekta: `.totem/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/totem/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Totem ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.totem/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/totem/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.totem/skills/git-release/SKILL.md` ovako:
...
Totem navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `totem.json`:
...
**Za ugradene agente** (u `totem.json`):
