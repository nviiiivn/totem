...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Totem dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Totem.
...
:::note
Ovaj alat je dostupan samo kada je `TOTEM_EXPERIMENTAL_LSP_TOOL=true` (ili `TOTEM_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Totem provajdera ili kada je varijabla `TOTEM_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Totem:
