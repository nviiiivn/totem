...
Možete dodati vanjske alate u Totem koristeći _Model Context Protocol_, ili MCP. Totem podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Totem Config](https://totem.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/totem`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Totem automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Totem će:
...
Ako server zahtijeva autentifikaciju, Totem će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `totem mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Totem sigurno čuva tokene u `~/.local/share/totem/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Totem, zajedno s ugrađenim alatima. Možete njima upravljati kroz Totem konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Totem sa vašim Sentry nalogom.
