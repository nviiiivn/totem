---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Totem.
---

Dodaci vam omogućavaju da proširite Totem spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Totem.
...
- `.totem/plugins/` - Dodaci na nivou projekta
- `~/.config/totem/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/totem/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/totem/totem.json`)
2. Konfiguracija projekta (`totem.json`)
3. Globalni direktorij dodataka (`~/.config/totem/plugins/`)
4. Direktorij dodataka projekta (`.totem/plugins/`)
...
Totem pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Totem SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Totem.
...
Spriječite totem da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u totem:
...
Pomoćnik `tool` kreira prilagođeni alat koji totem može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za totem zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://totem.ai/docs/sdk) za detalje.
