---
title: Konfiguracija
description: Korištenje Totem JSON konfiguracije.
---

Možete konfigurirati Totem koristeći JSON konfiguracijski fajl.

---

## Format

Totem podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/totem`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/totem/totem.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`TOTEM_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`totem.json` u projektu) - postavke specifične za projekat
5. **`.totem` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`TOTEM_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.totem` i `~/.config/totem` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/totem` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Totem konfiguraciju u `~/.config/totem/totem.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/totem/tui.json`.
...
Dodajte `totem.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Totem pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `TOTEM_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `TOTEM_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.totem` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.totem`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`totem.ai/config.json`**](https://totem.ai/config.json).

TUI konfiguracija koristi [**`totem.ai/tui.json`**](https://totem.ai/tui.json).
...
Koristite `TOTEM_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `totem.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `totem serve` i `totem web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Totem server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `totem.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Totem konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Totem pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/totem/agents/` ili `.totem/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Totem će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`totem run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/totem/commands/` ili `.totem/commands/`. [Saznajte više ovdje](/docs/commands).
...
Totem će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Totem **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Totem sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.totem/plugins/` ili `~/.config/totem/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Totem da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
