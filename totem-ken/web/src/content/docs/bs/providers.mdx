---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Totem.
---

import config from "../../../../config.mjs"
export const console = config.console

Totem koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Totem konfiguraciji.
...
u `~/.local/share/totem/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Totem
config.
...
Totem Zen je lista modela koje je obezbedio Totem tim koji su bili
testirano i potvrđeno da dobro radi sa Totem. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Totem Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite totem i idite na [totem.ai/auth](https://totem.ai/auth).
...
Radi kao i svaki drugi provajder u Totem i potpuno je opcionalan za korištenje.

---

## Totem Go

Totem Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Totem tim i koji su testirani i verificirani da dobro rade s Totem-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Totem Go` i idite na [totem.ai/auth](https://totem.ai/zen).
...
Da biste koristili Amazon Bedrock s Totem:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete totem:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `totem.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Totem.
:::
...
Možete konfigurirati totem za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi totem ispravno radio.
   :::
...
Također možete dodati modele kroz svoju totem konfiguraciju.
...
da pohrani token u totem auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Totem koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Totem
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`totem.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [totem-gitlab-auth](https://www.npmjs.com/package/totem-gitlab-auth).

##### Konfiguracija

Prilagodite putem `totem.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s totem:
...
Za korištenje Google Vertex AI s Totem:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće totem.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem totema, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [totem-helicone-session](https://github.com/H2Shami/totem-helicone-session) da automatski prijavite svaki Totem razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Totem razgovor naveden kao zasebna sesija.
...
Možete konfigurirati totem za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati totem za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati totem za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Totem. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/totem) za detalje.
:::
...
Da biste koristili Ollama Cloud s Totem:
...
3. Kopirajte API ključ za korištenje u Totem.
...
6. **Važno**: Prije upotrebe modela oblaka u Totem, morate lokalno povući informacije o modelu:
...
Totem Zen je lista testiranih i verifikovanih modela koju je obezbedio Totem tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Totem Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Totem Zen**.

```txt
   /connect
```

3. Unesite svoj Totem API ključ.
...
Također možete dodati dodatne modele putem vaše totem konfiguracije.
...
5. Također ih možete prilagoditi putem vaše totem konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Totemom:
...
Također možete prilagoditi modele kroz svoju totem konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s totem-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `totem.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Totem da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `totem auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Totem konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj totem konfiguraciji.
