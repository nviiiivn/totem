---
title: Server
description: Komunicirajte s totem serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `totem serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji totem klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `totem.local` |
...
Postavite `TOTEM_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `totem`, ili postavite `TOTEM_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `totem serve` i za `totem web`.
...
Kada pokrenete `totem`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite totem server za programsku interakciju sa totem-om.
:::

Ova arhitektura omogucava totem podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `totem serve` da startate standalone server. Ako je totem TUI vec pokrenut, `totem serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Totem [IDE](/docs/ide) pluginovi.
...
totem server izlaže sljedece API-je.
