...
U Totem mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Totem koristi nasu `totem` temu.
...
Totem dolazi sa vise ugradenih tema.
...
- Zele da Totem odgovara izgledu njihovog terminala
...
Totem podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/totem/themes/*.json` ili `$XDG_CONFIG_HOME/totem/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.totem/themes/*.json`
4. **Trenutni radni direktorij** - `./.totem/themes/*.json`
