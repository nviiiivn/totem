...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve totem route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "totem")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "totem")).toBe("openai")
    expect(statProvider("big-pickle", "", "totem")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "totem"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep totem as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "totem"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "totem"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep totem or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "totem"), country: "US" })).toMatchObject([
