...
const statsHomeTitle = "AI Model Usage Rankings | Totem Data"
const statsHomeDescription =
  "Explore Totem Go usage across AI models, including token volume, rankings, market share, token pricing, session cost, cache ratio, and geo breakdowns."
const statsHomeFallbackUrl = "https://totem.ai/data/"
...
function getStatsHomeUrl(base: string, requestUrl: string) {
  ...
  if (url.hostname === "stats.totem.ai") return "https://totem.ai/data/"
  if (url.hostname === "stats.dev.totem.ai") return "https://dev.totem.ai/data/"
  return url.toString()
}
  ...
  return (
    <section id="top-models" data-section="top-models">
      <h2 data-slot="top-models-title">
        <strong>Top models.</strong> <span>Usage of models across Totem Go.</span>
      </h2>
      <Show
...
function UniqueUsersSection(props: { data: StatsHomeData["users"] }) {
  ...
  return (
    <section id="unique-users" data-section="unique-users">
      ...
      <SectionTitle title="Unique Users" description="Daily unique Totem Go users by model." />
      <Show
        when={data().some((item) => usageTotal(item) > 0)}
        fallback={
