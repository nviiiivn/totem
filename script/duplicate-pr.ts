...
import { createTotem } from "@totem-ai/sdk"
...
`)
  ...
  const totem = await createTotem({ port: 0 })
  ...
  try {
    ...
    const session = await totem.client.session.create()
    const result = await totem.client.session
      .prompt({
