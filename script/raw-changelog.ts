...
const repo = process.env.GH_REPO ?? "anomalyco/totem"
const bot = ["actions-user", "github-actions[bot]", "totem", "totem-agent[bot]"]
const team = [
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- totem totem-adze/sdk totem/plugin totem/totem-faces/desktop totem/totem-faces/app sdks/vscode github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("totem/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("totem/")) areas.add("core")
      else if (file.startsWith("totem/totem-faces/desktop/src-tauri/")) areas.add("tauri")
      else if (file.startsWith("totem/totem-faces/desktop/") || file.startsWith("totem/totem-faces/app/")) areas.add("app")
      else if (file.startsWith("totem-adze/sdk/") || file.startsWith("totem/plugin/")) areas.add("sdk")
