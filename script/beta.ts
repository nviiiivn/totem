...
const model = "totem/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("totem")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with totem...`)
  ...
  try {
    await $`totem run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  totem failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with totem")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with totem...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `totem`.",
