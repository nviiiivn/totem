...
- Author package entrypoint is `@totem-ai/plugin/tui`.
...
```json
{
  "$schema": "https://totem.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/totem-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@totem-ai/plugin/tui`.
- `@totem-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running Totem version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.totem` is absent, no check is performed (backward compatible).
...
- `totem plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `totem plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.totem` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.totem`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.totem`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.totem` dir or `TOTEM_CONFIG_DIR`, Totem installs `@totem-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@totem-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Totem's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Totem registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Totem pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@totem-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"totem"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `totem.default`.
...
- Local plugins persist installed themes under the local `.totem/themes` area near the plugin config source.
...
- `--pure` / `TOTEM_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.totem/plugins/tui-smoke.tsx`
- Local vim plugin: `.totem/plugins/tui-vim.tsx`
- Local smoke config: `.totem/tui.json`
- Local smoke theme: `.totem/plugins/smoke-theme.json`
