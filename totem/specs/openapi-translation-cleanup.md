...
Trim `packages/totem/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/totem/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/totem`.
- `[x]` `bun typecheck` from `packages/totem`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/totem/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/totem`.
- `[x]` `bun dev generate > /tmp/totem-openapi.json` from `packages/totem`.
...
- Edit `packages/totem/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/totem-openapi.json` from `packages/totem`.
...
- Check source schemas in files like `packages/totem/src/session/schema.ts`, `packages/totem/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/totem/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/totem/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
