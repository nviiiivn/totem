...
Delete `packages/totem/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/totem/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/totem/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/totem`
- TUI plugin loader tests in `packages/totem` if runtime plugin API wiring changed
