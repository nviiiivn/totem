...
Practical reference for a future `packages/server` split after the totem
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/totem`.
...
Until enough shared service code lives outside `packages/totem`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/totem`

It should not import `packages/totem` services while `packages/totem`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on totem-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/totem` implementation details.
