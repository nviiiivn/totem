...
import { ModelsDev } from "@totem-ai/core/models-dev"
import { HttpRecorder } from "@totem-ai/http-recorder"
import { HttpRecorderInternal } from "@totem-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@totem-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@totem-ai/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.totem.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.TOTEM_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["totem", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("TOTEM_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("TOTEM_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["totem", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "totem-proxy",
    name: "Totem proxy",
    providerID: ProviderV2.ID.totem,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["totem", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.TOTEM_RECORD_CONSOLE_TOKEN && process.env.TOTEM_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.totem,
        name: "Totem Zen",
        env: ["TOTEM_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.TOTEM_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.TOTEM_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.TOTEM_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["totem", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("TOTEM_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("TOTEM_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("TOTEM_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set TOTEM_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the totem LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "totem.json"),
      JSON.stringify({ $schema: "https://totem.ai/config.json", ...scenario.config(model) }),
    ),
  )
