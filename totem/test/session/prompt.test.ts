...
import { SessionV1 } from "@totem-ai/core/v1/session"
import { Database } from "@totem-ai/core/database/database"
...
import * as SessionStop from "../../src/enforcement/session-stop"
import { Session } from "@/session/session"
import { SessionMessageTable } from "@totem-ai/core/session/sql"
...
import { SessionV2 } from "@totem-ai/core/session"
import { SessionExecution } from "@totem-ai/core/session/execution"
import { Skill } from "../../src/skill"
import { SystemPrompt } from "../../src/session/system"
import { Shell } from "@totem-ai/core/shell"
...
import { ProviderV2 } from "@totem-ai/core/provider"
import { ModelV2 } from "@totem-ai/core/model"
import { LocationServiceMap } from "@totem-ai/core/location-layer"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    path.join(dir, "totem.json"),
    JSON.stringify({ $schema: "https://totem.ai/config.json", ...config }),
  )
})
...
it.instance(
  "carve 4 — a redirect that lands before a held turn's tool calls run refuses them",
  () =>
    Effect.gen(function* () {
      const { llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      ...
      // discovered while building this test: totem dispatches multiple tool calls in
      // one turn CONCURRENTLY, not sequentially — so a delay inside one tool's body
      // (e.g. a bash sleep) does not create a window between the OTHER tool's check
      // and this one's. All of a turn's priority checks fire at essentially the same
      // instant, at the very start of the turn. So the real, reliable way to prove the
      // carve: hold the entire model response back with .wait(), land the redirect
      // while it's held, then release — every tool call in that turn must see the
      // redirect, since none of them can start until release.
      const gate = yield* Deferred.make<void>()
      yield* llm.push(reply().wait(deferredAsPromise(gate)).tool("bash", { command: "true" }).tool("glob", { pattern: "**/*.txt" }))

      const first = yield* prompt
        .prompt({ sessionID: session.id, agent: "build", model: ref, parts: [{ type: "text", text: "start" }] })
        .pipe(Effect.forkChild)

      yield* waitForBusy(session.id)

      const redirectID = MessageID.ascending()
      const second = yield* prompt
        .prompt({
          ...
          agent: "build",
          model: ref,
          parts: [{ type: "text", text: "stop, redirect" }],
        })
        .pipe(Effect.forkChild)
      ...
      yield* pollWithTimeout(
        sessions
          ...
          .pipe(
            Effect.map((msgs) =>
              msgs.some((msg) => msg.info.role === "user" && msg.info.id === redirectID) ? true : undefined,
            ),
          ),
        "timed out waiting for redirect message to save",
      )

      // NOW release the held response — bash and glob both start after the redirect exists
      yield* Deferred.succeed(gate, undefined)

      const [exitFirst, exitSecond] = yield* Effect.all([Fiber.await(first), Fiber.await(second)])
      expect(Exit.isSuccess(exitFirst)).toBe(true)
      expect(Exit.isSuccess(exitSecond)).toBe(true)

      const msgs = yield* sessions.messages({ sessionID: session.id })
      const assistant = msgs.find((m) => m.info.role === "assistant")
      if (!assistant) throw new Error("expected an assistant message")

      const bashPart = assistant.parts.find((p): p is SessionV1.ToolPart => p.type === "tool" && p.tool === "bash")
      const globPart = assistant.parts.find((p): p is SessionV1.ToolPart => p.type === "tool" && p.tool === "glob")

      expect(bashPart?.state.status).toBe("error")
      if (bashPart?.state.status === "error") expect(bashPart.state.error).toContain("newer human message")

      expect(globPart?.state.status).toBe("error")
      if (globPart?.state.status === "error") expect(globPart.state.error).toContain("newer human message")
    }),
  { git: true },
  10_000,
)

it.instance("carve 6 — durable todo state is injected into every turn's LLM input", () =>
  Effect.gen(function* () {
    const { llm } = yield* useServerConfig(providerCfg)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    const todo = yield* Todo.Service
    const session = yield* sessions.create({ title: "Carve 6" })

    yield* todo.update({
      sessionID: session.id,
      todos: [
        { content: "pending durable task", status: "pending", priority: "high" },
        { content: "finished durable task", status: "completed", priority: "low" },
      ],
    })
    ...
    const inputs = yield* llm.inputs
    const last = inputs.at(-1)?.messages
    if (!Array.isArray(last)) throw new Error("expected LLM messages")
    const userMsg = last.find((m: { role?: string }) => m?.role === "user")
    const content = JSON.stringify(userMsg)

    expect(content).toContain("pending durable task")
    expect(content).toContain("durable_store")
    expect(content).not.toContain("finished durable task")
  }),
)

it.instance("carve 6 — no injection when there are no active todos", () =>
  Effect.gen(function* () {
    const { llm } = yield* useServerConfig(providerCfg)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    const session = yield* sessions.create({ title: "Carve 6 empty" })
    ...
    if (!Array.isArray(last)) throw new Error("expected LLM messages")
    const userMsg = last.find((m: { role?: string }) => m?.role === "user")
    expect(JSON.stringify(userMsg)).not.toContain("durable_store")
  }),
)

it.instance("carve 5/3 — hardStop stops the session and blocks new work until resumed", () =>
  Effect.gen(function* () {
    const { llm } = yield* useServerConfig(providerCfg)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    const session = yield* sessions.create({ title: "Carve 5 stop" })

    expect(SessionStop.isStopped(session.id)).toBeUndefined()

    yield* prompt.hardStop(session.id, "test reason")

    const record = SessionStop.isStopped(session.id)
    expect(record).toBeDefined()
    expect(record?.reason).toBe("test reason")

    yield* llm.text("should never be reached")
    const exit = yield* prompt
      .prompt({ sessionID: session.id, agent: "build", model: ref, parts: [{ type: "text", text: "hello" }] })
      .pipe(Effect.exit)
    ...
    SessionStop.resume(session.id)
    expect(SessionStop.isStopped(session.id)).toBeUndefined()
  }),
)

it.instance("carve 6 — apology followed by the exact same mutation gets refused as a loop", () =>
  Effect.gen(function* () {
    ...
    const target = path.join(dir, "loop-target.txt")

    // round 1: apologizes in text AND writes the file
    yield* llm.push(reply().text("You're right, sorry about that — let me fix it.").tool("write", { filePath: target, content: "same content" }))
    // round 2: attempts the EXACT same write again, no fresh apology this round —
    // the apology from round 1's text is what should trigger the refusal here
    yield* llm.push(reply().tool("write", { filePath: target, content: "same content" }))

    yield* prompt.prompt({ sessionID: session.id, agent: "build", model: ref, parts: [{ type: "text", text: "go" }] })

    const msgs = yield* sessions.messages({ sessionID: session.id })
    const assistants = msgs.filter((m) => m.info.role === "assistant")
    expect(assistants.length).toBeGreaterThanOrEqual(2)

    const firstWrite = assistants[0].parts.find((p): p is SessionV1.ToolPart => p.type === "tool" && p.tool === "write")
    const secondWrite = assistants[1].parts.find(
      (p): p is SessionV1.ToolPart => p.type === "tool" && p.tool === "write",
    )

    expect(firstWrite?.state.status).toBe("completed")
    expect(secondWrite?.state.status).toBe("error")
    if (secondWrite?.state.status === "error") {
      expect(secondWrite.state.error).toContain("Loop detected")
    }
  }),
)
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/totem/src/session/processor.ts")
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderV2.ID.make("totem"), modelID: ModelV2.ID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
