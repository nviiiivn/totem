...
import type { HostPluginApi } from "@totem-ai/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  let selected = opts.theme?.selected ?? "totem"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "totem.default",
    ...
    state: {
      ...
      session: {
        ...
        enforcement: opts.state?.session?.enforcement ?? (() => undefined),
