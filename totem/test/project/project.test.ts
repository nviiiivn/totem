...
import { ProjectDirectories } from "@totem-ai/core/project/directories"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const totemFile = path.join(tmp, ".git", "totem")
      expect(yield* Effect.promise(() => Bun.file(totemFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "totem")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "totem")
      const wrongCache = path.join(parentDir, ".git", "totem")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "totem")
      const cacheB = path.join(bareB, "totem")
      const wrongCache = path.join(parentDir, ".git", "totem")
