...
it.instance("plan agent denies edits except .totem/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".totem/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".totem", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.TOTEM_TEST_HOME
      process.env.TOTEM_TEST_HOME = test.directory
