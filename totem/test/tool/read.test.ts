...
const githubBase = <A, E, R>(url: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.TOTEM_REPO_CLONE_GITHUB_BASE_URL
      process.env.TOTEM_REPO_CLONE_GITHUB_BASE_URL = url
      return previous
    }),
    () => self,
    (previous) =>
      Effect.sync(() => {
        if (previous) process.env.TOTEM_REPO_CLONE_GITHUB_BASE_URL = previous
        else delete process.env.TOTEM_REPO_CLONE_GITHUB_BASE_URL
      }),
  )
...
describe("tool.read external_directory permission", () => {
  ...
  it.live("carve 8 — repeat reads of the exact same file warn, different files don't", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped()
      yield* put(path.join(dir, "a.txt"), "content a")
      yield* put(path.join(dir, "b.txt"), "content b")
      const telemetryCtx = { ...ctx, sessionID: SessionID.make("ses_carve8_telemetry_test") }

      const first = yield* exec(dir, { filePath: path.join(dir, "a.txt") }, telemetryCtx)
      expect(first.output).not.toContain("[efficiency]")

      const second = yield* exec(dir, { filePath: path.join(dir, "a.txt") }, telemetryCtx)
      expect(second.output).toContain("[efficiency]")
      expect(second.output).toContain("2 times")

      const different = yield* exec(dir, { filePath: path.join(dir, "b.txt") }, telemetryCtx)
      expect(different.output).not.toContain("[efficiency]")
    }),
  )
