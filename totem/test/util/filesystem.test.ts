...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "totem.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "totem.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "totem.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "totem.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["totem.json", "totem.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "totem.json"),
        path.join(project, "totem.jsonc"),
      ])
    })
  })
