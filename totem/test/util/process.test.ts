...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.TOTEM_TEST ?? "")'), {
      env: {
        TOTEM_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "TOTEM_TEST_SHELL"], {
      shell: true,
      env: {
        TOTEM_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("TOTEM_TEST_SHELL=ok")
  })
