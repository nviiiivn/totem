...
describe("totem read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // TOTEM_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ totem }) =>
      Effect.gen(function* () {
        const r = yield* totem.spawn(["mcp", "list"])
        totem.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ totem }) =>
      Effect.gen(function* () {
        const r = yield* totem.spawn(["providers", "list"])
        totem.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ totem }) =>
      Effect.gen(function* () {
        const r = yield* totem.spawn(["models"])
        totem.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ totem }) =>
      Effect.gen(function* () {
        const r = yield* totem.spawn(["agent", "list"])
        totem.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh TOTEM_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ totem }) =>
      Effect.gen(function* () {
        const r = yield* totem.spawn(["session", "list"])
        totem.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ totem }) =>
      Effect.gen(function* () {
        const r = yield* totem.spawn(["stats"])
        totem.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under TOTEM_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ totem }) =>
      Effect.gen(function* () {
        const r = yield* totem.spawn(["db", "path"])
        totem.expectExit(r, 0, "db path")
