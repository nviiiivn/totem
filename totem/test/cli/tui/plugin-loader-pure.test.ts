...
`,
  ...
  const pure = process.env.TOTEM_PURE
  const meta = process.env.TOTEM_PLUGIN_META_FILE
  process.env.TOTEM_PURE = "1"
  process.env.TOTEM_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.TOTEM_PURE
    } else {
      process.env.TOTEM_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.TOTEM_PLUGIN_META_FILE
    } else {
      process.env.TOTEM_PLUGIN_META_FILE = meta
    }
  }
})
