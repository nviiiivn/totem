...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ totem }) =>
    Effect.gen(function* () {
      const result = yield* totem.spawn(["--replay-limit", "10"])

      totem.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ totem }) =>
    Effect.gen(function* () {
      const result = yield* totem.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ totem }) =>
    Effect.gen(function* () {
      const result = yield* totem.spawn(["--mini", "--port", "4096"])
