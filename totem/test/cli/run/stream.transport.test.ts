...
import { TotemClient, type GlobalEvent } from "@totem-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<TotemClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<TotemClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<TotemClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<TotemClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<TotemClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://totem.test"),
...
function sdk(
  input: {
    ...
    subscribe?: TotemClient["event"]["subscribe"]
    globalEvent?: TotemClient["global"]["event"]
    promptAsync?: TotemClient["session"]["promptAsync"]
    status?: TotemClient["session"]["status"]
    messages?: TotemClient["session"]["messages"]
    children?: TotemClient["session"]["children"]
    permissions?: TotemClient["permission"]["list"]
    questions?: TotemClient["question"]["list"]
  } = {},
) {
  const client = new TotemClient()

  const subscribe: TotemClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: TotemClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: TotemClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: TotemClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: TotemClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: TotemClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: TotemClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: TotemClient["question"]["list"] = input.questions ?? (() => ok([]))
