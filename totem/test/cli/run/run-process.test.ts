// Subprocess integration tests for `totem run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `totem.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `TOTEM_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("totem run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, totem }) =>
      Effect.gen(function* () {
        ...
        const result = yield* totem.run("say hi")
        totem.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, totem }) =>
      Effect.gen(function* () {
        ...
        const result = yield* totem.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, totem }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* totem.run("think", { extraArgs: ["--thinking"] })
        totem.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* totem.run("think again")
        totem.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, totem }) =>
      Effect.gen(function* () {
        ...
        const result = yield* totem.run("say hi", { format: "json" })
        totem.expectExit(result, 0)

        const events = totem.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ totem }) =>
      Effect.gen(function* () {
        const result = yield* totem.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, totem }) =>
      Effect.gen(function* () {
        ...
        const result = yield* totem.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, totem }) =>
      Effect.gen(function* () {
        ...
        const result = yield* totem.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, totem }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* totem.run("request permission", { permission: { bash: "ask" } })
        totem.expectExit(denied, 0)
        ...
        const allowed = yield* totem.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        totem.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* totem.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        totem.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, totem }) =>
      Effect.gen(function* () {
        ...
        const server = yield* totem.serve()

        const result = yield* totem.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, totem }) =>
      Effect.gen(function* () {
        const result = yield* totem.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, totem }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* totem.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
