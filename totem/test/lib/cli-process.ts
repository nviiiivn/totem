// Subprocess test harness for the totem CLI. Spawns the real binary against
...
//
// Configuration flows through totem's built-in test affordances:
//   - TOTEM_CONFIG_CONTENT      : provider config inline, no files to find
//   - TOTEM_TEST_HOME           : pins os.homedir() → tmpdir
//   - TOTEM_DISABLE_PROJECT_CONFIG : skip walking up for totem.json
//   - TOTEM_PURE                : skip external plugin discovery + install
//   - TOTEM_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `totem.run` is fully wired. The shape supports adding more
// builders (`totem.serve(opts)`, `totem.acp(opts)`, `totem.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of TotemCli.
import { test, type TestOptions } from "bun:test"
import { FSUtil } from "@totem-ai/core/fs-util"
import { AppProcess } from "@totem-ai/core/process"
...
const totemRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(totemRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    TOTEM_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    TOTEM_CONFIG_CONTENT: configJson,
    TOTEM_DISABLE_PROJECT_CONFIG: "1",
    TOTEM_PURE: "1",
    TOTEM_DISABLE_AUTOUPDATE: "1",
    TOTEM_DISABLE_AUTOCOMPACT: "1",
    TOTEM_DISABLE_MODELS_FETCH: "1",
    TOTEM_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `totem run`. New flags should
...
// `totem serve` is a long-lived process — it never exits on its own.
...
// `totem acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type TotemCli = {
  ...
  // Spawn `totem serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `totem acp` and return a duplex JSON-RPC handle. Long-lived:
  ...
  return Effect.gen(function* () {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          TOTEM_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("totem.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("totem.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "totem server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `totem serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("totem.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const totem: TotemCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, totem })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `totem.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "totem") {
...
// every call site. Use this for any test that needs the totem CLI fixture.
//
...
// resources (e.g. `totem.serve`) without an extra `Effect.scoped` wrapper —
