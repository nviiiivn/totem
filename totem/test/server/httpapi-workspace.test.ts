...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-totem-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-totem-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://totem.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-totem-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-totem-workspace")
