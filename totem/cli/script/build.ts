...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      TOTEM_CLI_NAME: `'${binary}'`,
      TOTEM_MODELS_DEV: modelsData,
      TOTEM_CHANNEL: `'${Script.channel}'`,
      TOTEM_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
    ...
    JSON.stringify(
      {
        ...
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/anomalyco/totem.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
