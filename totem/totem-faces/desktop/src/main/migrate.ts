...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.totem.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `totem.settings.dat` is special: it maps to the `totem.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"totem.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // totem.settings.dat → the electron settings store ("totem.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "totem.global.dat").
  const storeName = filename === "totem.settings.dat" ? "totem.settings" : filename
