...
import { expectTotemVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveTotem?: typeof resolveWslTotem
...
export function createWslServersController(
  ...
  const setTotemCheck = (distro: string, check: WslTotemCheck) => {
    setState({
      totemChecks: {
        ...state.totemChecks,
        [distro]: check,
  ...
  const checkTotem = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveTotem ?? resolveWslTotem)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return totemCheck(distro, resolved, version, appVersion)
  }

  const refreshTotemCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setTotemCheck(distro, await checkTotem(distro, opts))
  }
  ...
  const refreshTotemCheckBackground = (id: string, distro: string) => {
    void checkTotem(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setTotemCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl totem check failed", { id, distro, message })
      })
  }

  const refreshTotemChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkTotem(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setTotemCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl totem check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshTotemCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async probeTotem(name: string) {
      await runJob({ kind: "probe-totem", distro: name, startedAt: Date.now() }, async (abort) => {
        await refreshTotemCheck(name, { signal: abort.signal })
      })
    },

    async installTotem(name: string) {
      await runJob({ kind: "install-totem", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslTotem(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "Totem installation failed")
        }
        await refreshTotemCheck(name, { signal: abort.signal })
        expectTotemVersion(state.totemChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.totemChecks, distro) : {}),
      })
    },
...
function totemCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "totem is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "totem is installed but could not run",
    }
  }
  return {
    distro,
