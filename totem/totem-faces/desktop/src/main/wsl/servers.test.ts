...
let releaseTotemResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectTotemVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectTotemVersion("1.14.35", "1.16.2")).toThrow(
    "Totem update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Totem", () => {
  expect(
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.totem/bin/totem",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, totemChecks: {} })
})
...
test("ignores stale background Totem checks after removing a WSL server", async () => {
  persistedServers = []
  releaseTotemResolve = undefined
  ...
  await waitFor(() => !!releaseTotemResolve)
  await controller.removeServer("wsl:Debian")
  releaseTotemResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().totemChecks).toEqual({})
})

test("ignores stale startup Totem checks after removing a WSL server", async () => {
  ...
  return {
    ...
    resolveTotem: async () => {
      await new Promise<void>((resolve) => {
        releaseTotemResolve = resolve
      })
      return "/home/me/.totem/bin/totem"
    },
  }
}
