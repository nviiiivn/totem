import type { WslDistroProbe, WslTotemCheck, WslServerItem } from "../../preload/types"
...
export function clearWslDistroState(
  ...
  totemChecks: Record<string, WslTotemCheck>,
  distro: string,
) {
  const nextDistroProbes = { ...distroProbes }
  const nextTotemChecks = { ...totemChecks }
  delete nextDistroProbes[distro]
  delete nextTotemChecks[distro]
  return { distroProbes: nextDistroProbes, totemChecks: nextTotemChecks }
}
