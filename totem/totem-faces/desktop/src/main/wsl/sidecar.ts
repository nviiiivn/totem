...
import { type WslCommandLine, resolveWslTotem, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const totem = await resolveWslTotem(distro)
  if (!totem) throw new Error(`Totem is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "totem"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export TOTEM_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export TOTEM_CLIENT=desktop",
    `export TOTEM_SERVER_USERNAME=${shellEscape(username)}`,
    `export TOTEM_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(totem)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
