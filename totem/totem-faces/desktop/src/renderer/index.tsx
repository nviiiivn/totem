...
import type { UpdaterState } from "@totem-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@totem-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.TOTEM_CHANNEL === "prod" &&
...
const deepLinkEvent = "totem:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__TOTEM__ ??= {}
  const pending = window.__TOTEM__.deepLinks ?? []
  window.__TOTEM__.deepLinks = [...pending, ...urls]
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://totem.ai/favicon-96x96-v3.png",
      })
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("totem.global.dat").getItem("language")
