import { Button } from "@totem-ai/ui/button"
import { useDialog } from "@totem-ai/ui/context/dialog"
import { Spinner } from "@totem-ai/ui/spinner"
import { showToast } from "@totem-ai/ui/toast"
import { createEffect, createMemo, For, Match, onCleanup, Show, Switch } from "solid-js"
import { createStore } from "solid-js/store"
import { useLanguage } from "@/context/language"
import { usePlatform } from "@/context/platform"
import { useWslServers } from "./context"
import { enterWslTotemStep } from "./settings-model"

type WslServerStep = "wsl" | "distro" | "totem"

const STEPS: WslServerStep[] = ["wsl", "distro", "totem"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const totemCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.totemChecks[distro] ?? null
  })
  ...
  const totemReady = createMemo(() => {
    const check = totemCheck()
  ...
  const installingTotem = createMemo(() => {
    const job = current()?.job
    return job?.kind === "install-totem" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && totemReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-totem"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.totemChecks[distro]) {
      return { key: `probe-totem:${distro}`, run: () => api.probeTotem(distro) }
    }
    return null
  })
  ...
  const totemMessage = createMemo(() => {
    const state = current()
    if (!state) return language.t("wsl.onboarding.checkingTotem")
    const distro = selectedDistro()
    if (state.job?.kind === "install-totem") {
      return distro
        ? language.t("wsl.onboarding.updatingTotemIn", { distro })
        : language.t("wsl.onboarding.updatingTotem")
    }
    if (state.job?.kind === "probe-totem") {
      return distro
        ? language.t("wsl.onboarding.checkingTotemIn", { distro })
        : language.t("wsl.onboarding.checkingTotem")
    }
    if (totemCheck()?.error) return totemCheck()!.error
    if (totemCheck()?.matchesDesktop === false) {
      return distro
        ? language.t("wsl.onboarding.updateTotemIn", { distro })
        : language.t("wsl.onboarding.updateTotem")
    }
    if (totemReady()) {
      return distro
        ? language.t("wsl.onboarding.totemReadyIn", { distro })
        : language.t("wsl.onboarding.totemReady")
    }
    return distro
      ? language.t("wsl.onboarding.installTotemIn", { distro })
  ...
  const openTotemStep = () => {
    const distro = selectedDistro()
    if (!distro) return
    void run(() => enterWslTotemStep(distro, api.probeTotem, (step) => setStore("step", step)))
  }
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.totem"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : totemCheck()?.matchesDesktop === false
                  ? "warning"
                  : totemReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "totem"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.totem")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.probeTotem(distro))}
                      >
                        {language.t("wsl.onboarding.refresh")}
                      </Button>
                    </Show>
                    <Show when={!totemReady() || totemCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.installTotem(distro))}
                      >
                        <Show when={installingTotem()}>
                          <Spinner class="size-4 shrink-0" />
                        </Show>
                        {totemCheck()?.resolvedPath
                          ? language.t("wsl.onboarding.updateTotem")
                          : language.t("wsl.onboarding.installTotem")}
                      </Button>
                    </Show>
                  </div>
                </div>
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{totemMessage()}</div>
                <Show when={totemCheck()?.matchesDesktop === false ? totemCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "totem" && allReady() && selectedDistro()}>
