...
export type WslJob =
  ...
  | { kind: "probe-totem"; distro: string; startedAt: number }
  | { kind: "install-totem"; distro: string; startedAt: number }
...
export type WslServersPlatform = {
  ...
  probeTotem(name: string): Promise<void>
  installTotem(name: string): Promise<void>
