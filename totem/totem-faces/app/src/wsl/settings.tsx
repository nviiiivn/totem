...
import { Tag } from "@totem-ai/ui/v2/badge-v2"
import { ButtonV2 } from "@totem-ai/ui/v2/button-v2"
import { Dialog } from "@totem-ai/ui/v2/dialog-v2"
...
import { wslTotemAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.totemChecks[item.config.distro]
          const totemAction = () => wslTotemAction(check())
          const busy = () => wsl.data?.job?.kind === "install-totem" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={totemAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installTotem(item.config.distro))}
                    >
