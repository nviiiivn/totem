  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://totem.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://totem.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://totem.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          ...
          lastProject: { local: "/local", "https://totem.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://totem.example.com"),
      ),
    ).toEqual({
