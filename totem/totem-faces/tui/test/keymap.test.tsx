...
import { getTotemModeStack, TOTEM_BASE_MODE, TotemKeymapProvider, registerTotemKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerTotemKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when totem mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getTotemModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getTotemModeStack(keymap).push("autocomplete")
