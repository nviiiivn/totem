...
export const {
  ...
  init: () => {
    ...
    const [store, setStore] = createStore<{
      ...
      enforcement: {
        [sessionID: string]: EnforcementState
      }
      message: {
    ...
    event.subscribe((event, { workspace }) => {
      switch (event.type) {
        ...
        case "enforcement.updated":
          setStore("enforcement", event.properties.sessionID, event.properties.state)
          break
    ...
    const result = {
      ...
      session: {
        ...
        async sync(sessionID: string) {
          ...
          const task = (async () => {
            const [session, messages, todo, diff, enforcement] = await Promise.all([
              sdk.client.session.get({ sessionID }, { throwOnError: true }),
              sdk.client.session.messages({ sessionID, limit: 100 }),
              sdk.client.session.todo({ sessionID }),
              sdk.client.session.diff({ sessionID }),
              sdk.client.session.enforcement({ sessionID }),
            ])
            setStore(
              produce((draft) => {
                ...
                if (enforcement.data) draft.enforcement[sessionID] = enforcement.data
