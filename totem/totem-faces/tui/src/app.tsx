  ...
  const result = yield* Effect.scoped(
    Effect.gen(function* () {
      const renderer = yield* Effect.acquireRelease(
        Effect.tryPromise(() =>
          createCliRenderer({
            ...
            useMouse: !Flag.TOTEM_DISABLE_MOUSE && input.config.mouse,
            consoleOptions: {
      ...
      yield* Effect.acquireRelease(
        Effect.sync(() => registerTotemKeymap(keymap, renderer, input.config)),
      ...
      yield* Effect.tryPromise(async () => {
        ...
        // Splash paints as the renderer's first frame; the heavy provider
        // stack mounts one frame later underneath it, so init time is
        // covered instead of showing a black screen.
        const [appReady, setAppReady] = createSignal(false)
        const [heavyMounted, setHeavyMounted] = createSignal(false)
        setTimeout(() => setHeavyMounted(true), 30)
          ...
          return (
            ...
            >
              <EpilogueProvider set={(value) => (exit.epilogue = value)}>
                {process.env.TOTEM_FAST_BOOT ? null : <StartupLoading ready={appReady} />}
                {heavyMounted() ? (
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      <TuiStartupProvider
                        value={{
                          initialRoute: process.env.TOTEM_ROUTE ? JSON.parse(process.env.TOTEM_ROUTE) : undefined,
                          skipInitialLoading: Boolean(process.env.TOTEM_FAST_BOOT),
                        }}
                      >
                        <ClipboardProvider>
                          <TotemKeymapProvider keymap={keymap}>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost; onReady?: () => void }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.TOTEM_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.TOTEM_DISABLE_TERMINAL_TITLE) return
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.TOTEM_EXPERIMENTAL_WORKSPACES,
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          open("https://totem.ai/docs").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to Totem v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={
        !Flag.TOTEM_EXPERIMENTAL_DISABLE_COPY_ON_SELECT
          ? () => Selection.copy(renderer, toast, clipboard)
          : undefined
      }
    >
      <Show when={Flag.TOTEM_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
