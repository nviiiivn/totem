import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@totem-ai/plugin/tui"
...
import type { useTotemKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useTotemKeymap>
...
function stateApi(sync: ReturnType<typeof useSync>): TuiPluginApi["state"] {
  return {
    ...
    session: {
      ...
      enforcement(sessionID) {
        return sync.data.enforcement[sessionID]
      },
      messages(sessionID) {
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getTotemModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getTotemModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
