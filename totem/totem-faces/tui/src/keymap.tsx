...
export const TOTEM_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const TOTEM_MODE_KEY = "totem.mode"

export const TotemKeymapProvider = KeymapProvider
export const useTotemKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type TotemModeStack = ReturnType<typeof createTotemModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, TotemModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createTotemModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(TOTEM_MODE_KEY, TOTEM_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(TOTEM_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(TOTEM_MODE_KEY, stack.at(-1)?.mode ?? TOTEM_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? TOTEM_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(TOTEM_MODE_KEY, undefined)
...
export function useTotemModeStack() {
  return getTotemModeStack(useTotemKeymap())
}

export function getTotemModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Totem mode stack is not registered for this keymap")
  return value
}
...
export function registerTotemKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createTotemModeStack(keymap)
