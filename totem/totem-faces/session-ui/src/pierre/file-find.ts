...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("totem-find")
  api.delete("totem-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("totem-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("totem-find", new Highlight(...rest))
    return true
  }
