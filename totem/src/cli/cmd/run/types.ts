// Shared type vocabulary for the direct interactive mode (`totem --mini`).
//
...
import type { TotemClient, PermissionRequest, QuestionRequest, ToolPart } from "@totem-ai/sdk/v2"
import type { TuiConfig } from "@totem-ai/tui/config"
...
type PromptModel = Parameters<TotemClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<TotemClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<TotemClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<TotemClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<TotemClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<TotemClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<TotemClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<TotemClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<TotemClient["question"]["reject"]>[0]
