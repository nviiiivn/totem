import type { PermissionV1 } from "@totem-ai/core/v1/permission"
import { FSUtil } from "@totem-ai/core/fs-util"
// CLI entry point for `totem run` and `totem --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`totem --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`totem --mini --attach`): connects to a running
//      totem server and runs interactive mode against it.
//
...
import { createTotemClient, type TotemClient, type ToolPart } from "@totem-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<TotemClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run totem with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running totem server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to TOTEM_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to TOTEM_SERVER_USERNAME or 'totem')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: TotemClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: TotemClient, sessionID: string) {
      ...
      async function current(sdk: TotemClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: TotemClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: TotemClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: TotemClient) {
        ...
        async function loop(client: TotemClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createTotemClient({
        baseUrl: "http://totem.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
