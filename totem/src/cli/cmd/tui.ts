...
import { errorMessage } from "@totem-ai/tui/util/error"
import { withTimeout } from "@/util/timeout"
import { withNetworkOptions, resolveNetworkOptionsNoConfig } from "@/cli/network"
import { Filesystem } from "@/util/filesystem"
import type { GlobalEvent } from "@totem-ai/sdk/v2"
import type { EventSource } from "@totem-ai/tui/context/sdk"
import { writeHeapSnapshot } from "v8"
import { validateSession } from "../tui/validate-session"
import { win32InstallCtrlCGuard } from "@totem-ai/tui/terminal-win32"

declare global {
  const TOTEM_WORKER_PATH: string
}
...
async function target() {
  if (typeof TOTEM_WORKER_PATH !== "undefined") return TOTEM_WORKER_PATH
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start totem tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start totem in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
    ...
    try {
      ...
      try {
        ...
        await Effect.runPromise(
          ...
          console.error("=== TOTEM TUI EFFECT ERROR ===")
          console.error(error)
          if (error && typeof error === "object" && "stack" in error) {
            console.error((error as Error).stack)
          }
          console.error("=== END TOTEM TUI EFFECT ERROR ===")
        })
      } finally {
        await stop()
      }
