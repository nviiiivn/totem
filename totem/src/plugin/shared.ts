...
export const DEPRECATED_PLUGIN_PACKAGES = ["totem-openai-codex-auth", "totem-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, totemVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(totemVersion) || semver.major(totemVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.totem
  if (typeof range !== "string") return
  if (!semver.satisfies(totemVersion, range)) {
    throw new Error(`Plugin requires totem ${range} but running ${totemVersion}`)
  }
}
