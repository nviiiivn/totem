...
import { errorData, errorMessage } from "@totem-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@totem-ai/tui/context/theme"
import { Global } from "@totem-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@totem-ai/core/util/flock"
import { Flag } from "@totem-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@totem-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@totem-ai/core/v1/config/plugin"
import { createCommandShim } from "@totem-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@totem-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".totem"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".totem", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".totem", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.TOTEM_PURE ? [] : pluginOrigins
    if (Flag.TOTEM_PURE && pluginOrigins.length) {
    }
