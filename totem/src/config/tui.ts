...
import { isRecord } from "@totem-ai/tui/util/record"
...
import { TuiKeybind } from "@totem-ai/tui/config/keybind"
...
import { TuiConfig } from "@totem-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@totem/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old totem.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.totem`
  // folders between cwd and home, and TOTEM_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.TOTEM_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit TOTEM_TUI_CONFIG override, if set.
  if (Flag.TOTEM_TUI_CONFIG) {
    const configFile = Flag.TOTEM_TUI_CONFIG
  ...
  // 4. `.totem` directories (and TOTEM_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".totem") || dir === Flag.TOTEM_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".totem") && dir !== Flag.TOTEM_CONFIG_DIR) continue
