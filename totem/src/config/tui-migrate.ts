...
const TUI_SCHEMA_URL = "https://totem.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from totem.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const totem = await totemFiles(input)
  for (const file of totem) {
...
async function totemFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "totem"),
    ...(await Filesystem.findUp(["totem.json", "totem.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "totem"))
  }
  if (Flag.TOTEM_CONFIG) files.push(Flag.TOTEM_CONFIG)
