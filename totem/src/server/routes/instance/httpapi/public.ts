...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for totem serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(TotemHttpApi)
...
export const PublicApi = TotemHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "totem",
    version: "1.0.0",
    description: "totem api",
