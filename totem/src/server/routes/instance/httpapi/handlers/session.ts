...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const enforcement = Effect.fn("SessionHttpApi.enforcement")(function* (ctx: {
      params: { sessionID: SessionID }
    }) {
      yield* requireSession(ctx.params.sessionID)
      const stop = SessionStop.isStopped(ctx.params.sessionID)
      const directives = DirectiveStore.list(ctx.params.sessionID)
      return {
    ...
    const stop = Effect.fn("SessionHttpApi.stop")(function* (ctx: {
      ...
      yield* promptSvc.hardStop(ctx.params.sessionID, ctx.payload?.reason)
      return true
    })
    ...
    return handlers
      ...
      .handle("todo", todo)
      .handle("enforcement", enforcement)
      .handle("diff", diff)
