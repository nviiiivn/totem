  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context<Metadata>) =>
        run(params, ctx).pipe(
          Effect.map((result) => {
            const count = EfficiencyTelemetry.recordInvestigation(ctx.sessionID, "read", params)
            const warning = EfficiencyTelemetry.formatRepeatWarning("read", count)
            return warning ? { ...result, output: result.output + warning } : result
          }),
          Effect.orDie,
        ),
    }
