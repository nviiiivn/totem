...
import { Watcher } from "@totem-ai/core/filesystem/watcher"
...
import { assertWritable } from "@/enforcement/write-guard"
import { assertNotStopped } from "@/enforcement/directive-store"
import * as EfficiencyTelemetry from "@/enforcement/efficiency-telemetry"
import { assertNotLooping } from "@/enforcement/anti-loop"
...
export const ApplyPatchTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("ApplyPatchTool.execute")(function* (
      ...
      EfficiencyTelemetry.resetOnMutation(ctx.sessionID)

      for (const hunk of hunks) {
        const filePath = path.resolve(instance.directory, hunk.path)
        assertWritable(filePath)
        assertNotStopped(ctx.sessionID, filePath)
        assertNotLooping(ctx.sessionID, `apply_patch:${filePath}:${Bun.hash(JSON.stringify(hunk))}`)
        if (hunk.type === "update" && hunk.move_path) {
          assertWritable(path.resolve(instance.directory, hunk.move_path))
        }
