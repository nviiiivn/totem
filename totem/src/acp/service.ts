...
import type { Message, TotemClient, SessionMessageResponse } from "@totem-ai/sdk/v2"
...
export const AuthMethodID = "totem-login"
...
export class Service extends Context.Service<Service, Interface>()("@totem/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `totem auth login` in the terminal",
      name: "Login with totem",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: TotemClient) {
...
function makeUsageService(sdk: TotemClient) {
...
async function loadDirectorySnapshot(sdk: TotemClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, totem provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const totemProvider = providers[ProviderV2.ID.make("totem")]
  const totemModel = totemProvider ? Provider.sort(Object.values(totemProvider.models))[0] : undefined
  if (totemProvider && totemModel) return { providerID: totemProvider.id, modelID: totemModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Totem service failure", service })
}
