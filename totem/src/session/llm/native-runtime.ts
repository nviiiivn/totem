...
import type { LLMClientShape } from "@totem-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("totem"))
    return { type: "unsupported", reason: "provider is not openai, totem, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @totem-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains totem-owned. The native runtime only adapts
      // the @totem-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
