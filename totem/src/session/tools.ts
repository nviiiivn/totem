...
import * as PriorityResolver from "@/enforcement/priority-resolver"
...
export const resolve = Effect.fn("SessionTools.resolve")(function* (input: {
  ...
  // Carve 4 — priority resolver. The user message this turn is responding to. Checked
  // against a live read before every tool call so a mid-turn human redirect preempts
  // any further tool calls this turn had already planned.
  const sinceUserMessageID = input.messages.findLast((m) => m.info.role === "user")?.info.id
      ...
      execute(args, options) {
        return run.promise(
          Effect.gen(function* () {
            ...
            if (sinceUserMessageID) {
              const current = yield* sessionSvc.messages({ sessionID: input.session.id }).pipe(Effect.orDie)
              const hasNewer = current.some((m) => m.info.role === "user" && m.info.id > sinceUserMessageID)
              PriorityResolver.assertNoNewerHumanInput(hasNewer, sinceUserMessageID)
            }
