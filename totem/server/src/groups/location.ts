...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-totem-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-totem-directory"] ? decode(request.headers["x-totem-directory"]) : process.cwd())
