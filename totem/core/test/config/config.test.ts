...
import { ConfigProvider } from "@totem-ai/core/config/provider"
import { ConfigMigrateV1 } from "@totem-ai/core/v1/config/migrate"
...
import { Policy } from "@totem-ai/core/policy"
...
describe("Config", () => {
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(
                path.join(tmp.path, "totem.json"),
                JSON.stringify({ $schema: "middle", providers: { middle: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "totem.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "totem.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(path.join(tmp.path, "totem.json"), "{ invalid"),
              fs.writeFile(path.join(tmp.path, "totem.jsonc"), JSON.stringify({ providers: { invalid: true } })),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .totem configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".totem"), { recursive: true })
            await fs.mkdir(path.join(directory, ".totem"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "totem.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "totem.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "totem.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "totem.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "config.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".totem", "totem.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".totem", "totem.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".totem")),
              AbsolutePath.make(path.join(directory, ".totem")),
            ])
