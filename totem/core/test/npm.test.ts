...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@totem/acme")).toBe("@totem/acme")
    expect(Npm.sanitize("@totem/acme@1.0.0")).toBe("@totem/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/totem/acme.git"
    const expected = win ? "acme@git+https_//github.com/totem/acme.git" : spec
