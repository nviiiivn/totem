...
const snapshot = path.join(root, "totem/core/schema.json")
const tsDir = path.join(root, "totem/core/src/database/migration")
const registry = path.join(root, "totem/core/src/database/migration.gen.ts")
const schema = path.join(root, "totem/core/src/database/schema.gen.ts")
...
async function generate() {
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), "totem-core-migration-"))
...
async function check() {
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), "totem-core-migration-check-"))
  const incremental = path.join(temporary, "incremental")
  const full = path.join(temporary, "full")
  try {
    await fs.mkdir(incremental)
    ...
    if ((await generatedMigrations(incremental)).length > 0) {
      throw new Error(
        "Core schema has ungenerated database migrations. Run `bun script/migration.ts` from totem/core.",
      )
    }
    ...
    if ((await Bun.file(schema).text()) !== (await formatTypescript(renderSchema(await generatedSql(full))))) {
      throw new Error("Current database schema is stale. Run `bun script/migration.ts` from totem/core.")
    }
    ...
    if ((await Bun.file(registry).text()) !== (await formatTypescript(renderRegistry(migrations)))) {
      throw new Error("Database migration registry is stale. Run `bun script/migration.ts` from totem/core.")
    }
  } finally {
    await fs.rm(temporary, { recursive: true, force: true })
  }
...
async function drizzle(temporary: string, output: string, name?: string) {
  ...
  await Bun.write(
    config,
    `import config from ${JSON.stringify(pathToFileURL(path.join(root, "totem/core/drizzle.config.ts")).href)}

export default { ...config, out: ${JSON.stringify(output)} }
`,
  )
  await $`bun drizzle-kit generate --config ${config} ${name ? ["--name", name] : []}`.cwd(
    path.join(root, "totem/core"),
  )
}
