...
import { ToolFailure } from "@totem-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@totem/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.TOTEM_WEBSEARCH_PROVIDER === "exa" || process.env.TOTEM_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.TOTEM_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("TOTEM_EXPERIMENTAL") || truthy("TOTEM_ENABLE_EXA") || truthy("TOTEM_EXPERIMENTAL_EXA"),
    enableParallel: truthy("TOTEM_ENABLE_PARALLEL") || truthy("TOTEM_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `totem/${InstallationVersion}`,
