...
import type { IntegrationOAuthMethodRegistration } from "@totem-ai/plugin/v2/effect/integration"
import { define } from "@totem-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@totem-ai/sdk/v2/types"
...
export const TotemPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "totem",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("TotemPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("totem")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Totem provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
