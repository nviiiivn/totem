...
| `~/.config/opencode/totempole.json` | Plugin settings - agents, multiplexer, MCPs, council |
| `~/.config/opencode/totempole.jsonc` | Same, but with JSONC (comments + trailing commas). Takes precedence over `.json` if both exist |
| `.opencode/totempole.json` | Project-local overrides (optional, higher precedence than user config) |
...
Customize agent prompts without modifying source code. Create markdown files in `~/.config/opencode/totempole/`:
