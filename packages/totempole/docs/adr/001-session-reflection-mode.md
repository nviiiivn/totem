...
We are adding a `--sessions` mode to the `/reflect` command in totempole. This mode enables cross-session reflection by analyzing past OpenCode sessions to find repeated patterns, friction, and improvement opportunities.
...
**Decision:** Store reflection summaries in `~/.config/opencode/totempole/reflections/`.
...
**Structure:**
```
~/.config/opencode/totempole/reflections/
  sessions/
...
|--------|------|------|----------|
| `~/.config/opencode/totempole/reflections/` | Existing directory, single location | Mixes config and data | **Accepted** |
