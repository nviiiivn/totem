...
Built-in Model Context Protocol (MCP) servers ship with totempole and give agents access to external tools - web search, library documentation, and code search.
...
Control which MCPs each agent can use via the `mcps` array in your preset config (`~/.config/opencode/totempole.json` or `.jsonc`):
