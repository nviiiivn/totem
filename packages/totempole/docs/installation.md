...
```
[i] Configuration already exists at ~/.config/opencode/totempole.json. Use --reset to overwrite.
```
...
`~/.config/opencode/.totempole/skill-updates/` for manual review.
...
Open your generated config at `~/.config/opencode/totempole.json`
...
> **💡 Tip: Models are fully customizable.** The installer sets sensible defaults, but you can assign *any* model to *any* agent. Edit `~/.config/opencode/totempole.json` (or `.jsonc` for comments support) to override models, adjust reasoning effort, or disable agents entirely.
...
bunx totempole@latest install --preset=opencode-go
...
- They can easily assign **different models to different agents** by editing `~/.config/opencode/totempole.json` (or `.jsonc`).
...
- Read the generated `~/.config/opencode/totempole.json` (or `.jsonc`) file to understand the current configuration.
...
- `~/.config/opencode/totempole.json` (or `.jsonc`)
...
3. Verify your config file has the correct provider configuration:
   ```bash
   cat ~/.config/opencode/totempole.json
   ```
...
1. **Remove the plugin from your OpenCode config**:
   ...
   Edit `~/.config/opencode/opencode.json` and remove `"totempole"` from the `plugin` array.

2. **Remove configuration files (optional)**:
   ```bash
   rm -f ~/.config/opencode/totempole.json
   rm -f ~/.config/opencode/totempole.json.bak
   ```
...
3. **Remove skills (optional)**:
   ...
   rm -rf ~/.config/opencode/skills/totempole
   ```
