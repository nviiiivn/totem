...
You can enable the companion by adding a `companion` section to your setting configuration file (`~/.config/opencode/totempole.json` or `.opencode/totempole.json`):
...
```text
$XDG_DATA_HOME/opencode/storage/totempole/bin/totempole-companion
```

If `XDG_DATA_HOME` is unset, this resolves to:

```text
~/.local/share/opencode/storage/totempole/bin/totempole-companion
```
