...
Centralizes configuration loading, validation, schema definitions, and runtime state management for the totempole plugin. This folder implements the configuration pipeline that merges user preferences, project overrides, and preset-based agent configurations, providing validated runtime configuration objects to the rest of the plugin.
...
```
1. Discovery Phase
   ├─ User config: $OPENCODE_CONFIG_DIR/totempole.{jsonc,json}
   ├─ Project config: <directory>/.opencode/totempole.{jsonc,json}
   └─ Environment variable: TEMPPOLE_PRESET (overrides preset field)
...
- `TEMPPOLE_PRESET`: Overrides config.preset at runtime
