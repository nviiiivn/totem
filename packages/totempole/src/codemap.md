...
1. **State Path Resolution**: Determines XDG-compliant state directory (`~/.local/share/opencode/storage/totempole/tui-state.json`)
...
- Plugin config loaded via `loadPluginConfig()` with support for:
  - User overrides from `~/.config/opencode/totempole.json`
