...
```text
~/.config/opencode/totempole.jsonc
```
...
| `~/.config/opencode/totempole.jsonc` | User plugin config with comments/trailing commas; takes precedence over `.json` |
| `~/.config/opencode/totempole.json` | User plugin config generated by installer |
| `<project>/.opencode/totempole.json` | Project-local plugin overrides checked before user config |
| `~/.config/opencode/totempole/{agent}.md` | Full prompt replacement for a built-in agent |
| `~/.config/opencode/totempole/{agent}_append.md` | Append-only prompt tuning for a built-in agent |
| `~/.config/opencode/totempole/{preset}/{agent}.md` | Preset-specific full prompt replacement |
| `~/.config/opencode/totempole/{preset}/{agent}_append.md` | Preset-specific append-only prompt tuning |
...
1. If a `preset` is active, check
   `~/.config/opencode/totempole/{preset}/` first.
2. Fall back to `~/.config/opencode/totempole/`.
...
```text
~/.config/opencode/totempole/orchestrator_append.md
```

or for only the `openai` preset:

```text
~/.config/opencode/totempole/openai/orchestrator_append.md
```
...
```text
~/.config/opencode/totempole/orchestrator.md
```
...
~/.config/opencode/totempole/orchestrator_append.md. Apply it?
```
