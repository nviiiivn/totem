...
export default new Hono<{ Bindings: Env }>()
  ...
  .post("/exchange_github_app_token", async (c) => {
    const EXPECTED_AUDIENCE = "totem-github-action"
  ...
  /**
   * Used by the GitHub action to get GitHub installation access token given user PAT token (used when testing `totem github run` locally)
   */
  ...
  /**
   * Used by the totem CLI to check if the GitHub app is installed
   */
