...
Both `action` and `resource` use totem's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.totem` files is intentionally deferred until `.totem` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether totem permits an operation through its managed execution paths.
