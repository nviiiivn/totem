...
The totem server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@totem-ai/llm` tool loop after replacing its
...
- plugins get global "totem" instance like in that post i showed
- totem instance has stuff like `totem.session.prompt()` or
  `totem.tool.register({...})`
...
- decide whether to preserve deprecated `@totem-ai/llm` orchestration exports
