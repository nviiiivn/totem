# Remove `packages/totem/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/totem/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the totem legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/totem/src/storage/db.ts` are concentrated in 22 source files:

- `packages/totem/src/account/repo.ts`
- `packages/totem/src/cli/cmd/db.ts`
- `packages/totem/src/cli/cmd/import.ts`
- `packages/totem/src/cli/cmd/stats.ts`
- `packages/totem/src/control-plane/workspace.ts`
- `packages/totem/src/index.ts`
- `packages/totem/src/node.ts`
- `packages/totem/src/permission/index.ts`
- `packages/totem/src/project/project.ts`
- `packages/totem/src/server/projectors.ts`
- `packages/totem/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/totem/src/server/shared/fence.ts`
- `packages/totem/src/session/message-v2.ts`
- `packages/totem/src/session/projectors.ts`
- `packages/totem/src/session/prompt.ts`
- `packages/totem/src/session/session.ts`
- `packages/totem/src/session/todo.ts`
- `packages/totem/src/share/share-next.ts`
- `packages/totem/src/storage/db.ts`
- `packages/totem/src/sync/index.ts`
- `packages/totem/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy totem database wrapper; `packages/totem/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the totem wrapper.
...
Status: Completed. `SyncEvent` and the totem projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy totem database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy totem database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy totem database wrapper.
...
All migration groups are complete or superseded. `packages/totem/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No totem data-migration group remains.

The previous totem `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate totem data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/totem`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/totem/src`
- `bun typecheck` from `packages/totem`
- Relevant package tests from `packages/totem`, not the repo root
