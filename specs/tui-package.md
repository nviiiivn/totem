...
Move the canonical Totem terminal application from
`packages/totem/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/totem`,
`packages/cli`, or `@totem-ai/core`.

The SDK is the TUI's Totem boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/totem`.
- Do not replace private `packages/totem` imports with `@totem-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@totem-ai/tui`.
...
- Totem domain data displayed by the TUI
...
```text
packages/totem/src/cli/cmd/tui
```

Its private dependency on `packages/totem` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/totem/src/*`.
...
Status: Completed. The private `@totem-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@totem-ai/tui`.
...
- The package has no dependency on `totem`, `@totem-ai/cli`, or
  `@totem-ai/core`.
...
primitives now live in `@totem-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@totem-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Totem globals or process state;
...
Replace process-global Totem state with resolved TUI inputs.
...
`@totem-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/totem` initially.
...
canonical in `@totem-ai/tui`. Configured references resolve through the new
...
adapters rather than Totem domain implementations.

Make the SDK the only Totem domain boundary used by the TUI.
...
- Domain-facing TUI code imports Totem data and operations only from
  `@totem-ai/sdk`.
...
`@totem-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/totem`.
...
- The package has no imports from `packages/totem`, `packages/cli`, or
  `@totem-ai/core`.
...
public `@totem-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/totem`.
- Keep the legacy embedded worker and server startup in `packages/totem`.
- Change those adapters to load config, create transport inputs, and call the
  public `@totem-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/totem`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@totem-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@totem-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Totem domain state.
...
cd packages/totem && bun typecheck
...
rg '@totem-ai/core|packages/totem|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/totem/src packages/totem/test
```
...
- Build the current-platform `packages/totem` binary.
